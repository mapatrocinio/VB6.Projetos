...
Private Sub cmdRelatorio_Click()
  ...
  'Captura total de taxa de cartões
  strSql = "SELECT Sum(cons_t_cred.VrCalcDescCartTxAdm) As VALOR " & _
    "FROM TURNO LEFT JOIN cons_t_cred ON TURNO.PKID = CONS_T_CRED.PKID " & _
  ...
  strSql = "SELECT Sum(cons_t_penhor.PgtoPenhor) As VALOR " & _
    "FROM TURNO LEFT JOIN cons_t_penhor ON TURNO.PKID = cons_t_penhor.PKID " & _
    "WHERE TURNO.DATA >= " & Formata_Dados(mskData(0).Text & " 00:00", tpDados_DataHora, tpNulo_Aceita) & _
    " AND TURNO.DATA <= " & Formata_Dados(mskData(1).Text & " 23:59", tpDados_DataHora, tpNulo_Aceita)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If objRs.EOF Then
...
Private Sub Form_Load()
  ...
  '
  Report1.ReportFileName = gsReportPath & "ResumoDespesasNovo.rpt"
