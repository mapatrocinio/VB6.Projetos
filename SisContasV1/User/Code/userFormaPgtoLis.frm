...
Begin VB.Form frmUserFormaPgtoLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userFormaPgtoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   4440
   End
End
Attribute VB_Name = "frmUserFormaPgtoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma forma de pagamento !", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmUserFormaPgtoInc.Status = tpStatus_Alterar
  frmUserFormaPgtoInc.lngFORMAPGTOID = grdGeral.Columns("ID").Value
  frmUserFormaPgtoInc.Show vbModal
  
  If frmUserFormaPgtoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objFormaPgto  As busSisContas.clsFormaPgto
  ...
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    MsgBox "Selecione uma forma de pagamento para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da forma de pagamento " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  Set objGeral = New busSisContas.clsGeral
  strSql = "SELECT * FROM DESPESA WHERE FORMAPGTOID = " & Formata_Dados(CLng(grdGeral.Columns("ID").Value), tpDados_Longo, tpNulo_Aceita)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Não é possível excluir esta forma de pagamento, pois existem despesas cadastradas para ela.", "[cmdExcluir_Click]"
    Exit Sub
  End If
  objRs.Close
  Set objRs = Nothing
  Set objGeral = Nothing
  'OK
  Set objFormaPgto = New busSisContas.clsFormaPgto
  
  objFormaPgto.ExcluirFormaPgto CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserFormaPgtoInc.Status = tpStatus_Incluir
  frmUserFormaPgtoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT FORMAPGTO.PKID, FORMAPGTO.FORMAPGTO " & _
            "FROM FORMAPGTO " & _
            " ORDER BY FORMAPGTO.FORMAPGTO "
  '
