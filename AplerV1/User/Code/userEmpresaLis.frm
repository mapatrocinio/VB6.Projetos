...
Private Sub cmdExcluir_Click()
  ...
  '
  strSql = "Select * from TITULAR where EMPRESAID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Empresa não pode ser excluida, pois já está associada a titular(es).", "frmUserEmpresaLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão da empresa " & grdGeral.Columns("Empresa").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT EMPRESA.PKID, EMPRESA.NOME, EMPRESA.CODIGO " & _
            "FROM EMPRESA " & _
            "ORDER BY NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserEmpresaLis.grdGeral_UnboundReadDataEx]"
End Sub
