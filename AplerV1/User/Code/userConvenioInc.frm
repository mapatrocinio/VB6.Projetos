...
Begin VB.Form frmUserConvenioInc 
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      Begin TrueDBGrid60.TDBGrid grdPlano 
         Height          =   4545
         Left            =   -74910
         OleObjectBlob   =   "userConvenioInc.frx":0054
         TabIndex        =   26
         Top             =   390
         Width           =   7965
      End
...
Private Sub cmdExcluir_Click()
  On Error GoTo trata
  Dim objConvenioConvenio     As busApler.clsPlanoConvenio
  '
  Select Case tabDetalhes.Tab
  Case 2 'Exclusão de Plano
    '
    If Len(Trim(grdPlano.Columns("PKID").Value & "")) = 0 Then
      MsgBox "Selecione um Plano do convênio.", vbExclamation, TITULOSISTEMA
      SetarFoco grdPlano
      Exit Sub
    End If
    '
    Set objConvenioConvenio = New busApler.clsPlanoConvenio
    '
    If MsgBox("Confirma exclusão do plano " & grdPlano.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
      SetarFoco grdPlano
      Exit Sub
    End If
    'OK
    objConvenioConvenio.ExcluirPlanoConvenio CLng(grdPlano.Columns("PKID").Value)
    '
    PLANO_MontaMatriz
    grdPlano.Bookmark = Null
    grdPlano.ReBind
    ...
    Set objConvenioConvenio = Nothing
    SetarFoco grdPlano
  End Select
  Exit Sub
trata:
...
Private Sub cmdOK_Click()
  ...
  If Not objRs.EOF Then
    ...
    TratarErroPrevisto "Convênio já cadastrado"
    objRs.Close
    Set objRs = Nothing
  ...
  ElseIf Status = tpStatus_Incluir Then
    ...
    Set objRs = objConvenio.SelecionarConvenioPeloNome(txtNome.Text)
    If Not objRs.EOF Then
...
Private Function ValidaCampos() As Boolean
  ...
  If Not Valida_String(txtNumeroContrato, TpObrigatorio, blnSetarFocoControle) Then
    strMsg = strMsg & "Preencher o número do contrato" & vbCrLf
    tabDetalhes.Tab = 0
  End If
  ...
  If Not Valida_String(cboTipoConvenio, TpObrigatorio, blnSetarFocoControle) Then
    strMsg = strMsg & "Selecionar o tipo do convênio" & vbCrLf
    tabDetalhes.Tab = 0
  End If
  If Not Valida_Data(mskDtInicio, TpObrigatorio, blnSetarFocoControle) Then
    strMsg = strMsg & "Preencher a data inicial válida" & vbCrLf
    tabDetalhes.Tab = 0
  End If
  If Not Valida_Data(mskDtFim, TpNaoObrigatorio, blnSetarFocoControle) Then
    strMsg = strMsg & "Preencher a data final válida" & vbCrLf
    tabDetalhes.Tab = 0
  End If
