...
Begin VB.Form frmUserConvenioLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de convênio"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10080
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userConvenioLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8160
   End
End
Attribute VB_Name = "frmUserConvenioLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um convênio !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserConvenioInc.Status = tpStatus_Alterar
  frmUserConvenioInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserConvenioInc.Show vbModal
  
  If frmUserConvenioInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um convênio para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do convênio " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objConvenio = New busApler.clsConvenio
  objConvenio.ExcluirConvenio CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserConvenioInc.Status = tpStatus_Incluir
  frmUserConvenioInc.lngPKID = 0
  frmUserConvenioInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT CONVENIO.PKID, CONVENIO.NOME, CONVENIO.NUMEROCONTRATO, CONVENIO.CODIGO "
  strSql = strSql & " FROM CONVENIO " & _
            " ORDER BY CONVENIO.NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserConvenio.grdGeral_UnboundReadDataEx]"
End Sub
