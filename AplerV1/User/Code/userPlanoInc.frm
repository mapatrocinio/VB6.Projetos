...
Begin VB.Form frmUserPlanoInc 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cadastro de Plano"
   ClientHeight    =   4770
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10290
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      TabPicture(0)   =   "userPlanoInc.frx":0000
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Faixas"
      TabPicture(1)   =   "userPlanoInc.frx":001C
      ...
      Begin TrueDBGrid60.TDBGrid grdFaixa 
         Height          =   3945
         Left            =   -74910
         OleObjectBlob   =   "userPlanoInc.frx":0046
         TabIndex        =   4
         Top             =   480
         Width           =   7545
      End
   End
End
Attribute VB_Name = "frmUserPlanoInc"
...
trata:
  Err.Raise Err.Number, _
            "[frmUserPlanoInc.LimparCampos]", _
            Err.Description
End Sub
...
Private Sub cmdAlterar_Click()
  ...
  Case 1
    ...
    frmUserValorPlanoInc.lngPKID = grdFaixa.Columns("PKID").Value
    frmUserValorPlanoInc.lngTIPOPLANOID = lngPKID
    frmUserValorPlanoInc.strDescrPlano = txtDescricao.Text
    frmUserValorPlanoInc.Status = tpStatus_Alterar
    frmUserValorPlanoInc.Show vbModal

    If frmUserValorPlanoInc.blnRetorno Then
      FAIXA_MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  Case 1 'Exclusão de Plano
    ...
    End If
    'OK
    objValorPlano.ExcluirValorPlano CLng(grdFaixa.Columns("PKID").Value)
    '
    FAIXA_MontaMatriz
    grdFaixa.Bookmark = Null
    grdFaixa.ReBind
...
Private Sub cmdIncluir_Click()
  ...
  Select Case tabDetalhes.Tab
  Case 1
    frmUserValorPlanoInc.Status = tpStatus_Incluir
    frmUserValorPlanoInc.lngPKID = 0
    frmUserValorPlanoInc.lngTIPOPLANOID = lngPKID
    frmUserValorPlanoInc.strDescrPlano = txtDescricao.Text
    frmUserValorPlanoInc.Show vbModal
...
Private Sub cmdOK_Click()
  Dim objPlano                  As busApler.clsPlano
  ...
  Set objPlano = New busApler.clsPlano
  'Status
  ...
  strSql = "SELECT * FROM PLANO " & _
    " WHERE PLANO.DESCRICAO = " & Formata_Dados(txtDescricao.Text, tpDados_Texto) & _
    " AND PLANO.PKID <> " & Formata_Dados(lngPKID, tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    Pintar_Controle txtDescricao, tpCorContr_Erro
    TratarErroPrevisto "Plano já cadastrado"
    objRs.Close
    Set objRs = Nothing
  ...
  ElseIf Status = tpStatus_Incluir Then
    'Inserir Plano
    objPlano.InserirPlano txtDescricao.Text, _
                          txtObservacao.Text
    blnRetorno = True
    'Selecionar plano cadastrado
    Set objRs = objPlano.SelecionarPlanoPelaDescricao(txtDescricao.Text)
    If Not objRs.EOF Then
...
Private Function ValidaCampos() As Boolean
  ...
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserPlanoInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
trata:
  Err.Raise Err.Number, _
            Err.Source & ".[frmUserPlanoInc.ValidaCampos]", _
            Err.Description
End Function
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserPlanoInc.Form_Activate]"
End Sub
...
Private Sub Form_Load()
  ...
  ElseIf Status = tpStatus_Alterar Or Status = tpStatus_Consultar Then
    Set objPlano = New busApler.clsPlano
    Set objRs = objPlano.SelecionarPlanoPeloPkid(lngPKID)
    '
    If Not objRs.EOF Then
...
Public Sub FAIXA_MontaMatriz()
  ...
  '
  strSql = "SELECT VALORPLANO.PKID, VALORPLANO.DESCRICAO, VALORPLANO.VALORTITULAR, VALORPLANO.VALORDEPENDENTE, CASE VALORPLANO.STATUS WHEN 'A' THEN 'Ativo' ELSE 'Inativo' END  " & _
          "FROM VALORPLANO " & _
          "WHERE VALORPLANO.TIPOPLANOID = " & lngPKID & _
          " ORDER BY VALORPLANO.DESCRICAO"
