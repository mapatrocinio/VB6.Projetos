...
Begin VB.Form frmUserLinhaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Linha"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userLinhaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserLinhaLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmUserLinhaInc.lngLINHAID = grdGeral.Columns("ID").Value
  frmUserLinhaInc.Status = tpStatus_Alterar
  frmUserLinhaInc.Show vbModal
  
  If frmUserLinhaInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objLinha          As busApler.clsLinha
  ...
  On Error GoTo trata
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma linha para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from TAB_TITLINHA where LINHAID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Linha não pode ser excluida, pois já está associada a associado.", "frmUserGarcomLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão da linha " & grdGeral.Columns("Linha").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objLinha = New busApler.clsLinha
  
  objLinha.ExcluirLinha CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserLinhaInc.Status = tpStatus_Incluir
  frmUserLinhaInc.lngLINHAID = 0
  frmUserLinhaInc.Show vbModal
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserLinhaLis.grdGeral_UnboundReadDataEx]"
End Sub
