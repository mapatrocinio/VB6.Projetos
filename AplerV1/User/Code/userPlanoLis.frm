...
Begin VB.Form frmUserPlanoLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userPlanoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8160
   End
End
Attribute VB_Name = "frmUserPlanoLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmUserPlanoInc.Status = tpStatus_Alterar
  frmUserPlanoInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserPlanoInc.Show vbModal
  
  If frmUserPlanoInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  'Cadastro de Planos
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um plano para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do plano " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objPlano = New busApler.clsPlano
  objPlano.ExcluirPlano CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserPlanoInc.Status = tpStatus_Incluir
  frmUserPlanoInc.lngPKID = 0
  frmUserPlanoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de Planos
  strSql = "SELECT PLANO.PKID, PLANO.DESCRICAO, CASE PLANO.STATUS WHEN 'A' THEN 'Ativo' ELSE 'Inativo' END "
  strSql = strSql & " FROM PLANO " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserPlano.grdGeral_UnboundReadDataEx]"
End Sub
