...
Begin VB.Form frmUserTipoSocioInc 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Inclusão do tipo de sócio"
   ClientHeight    =   2565
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10380
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      Tabs            =   1
      TabHeight       =   520
      TabCaption(0)   =   "&Dados do Tipo de Sócio"
      TabPicture(0)   =   "userTipoSocioInc.frx":0000
   ...
   End
End
Attribute VB_Name = "frmUserTipoSocioInc"
...
Private Sub cmdOK_Click()
  ...
  Case 0 'Inclusão/Alteração de Grupo cardápio
    ...
    strSql = "Select * From TIPOSOCIO WHERE DESCRICAO = " & Formata_Dados(txtDescricao.Text, tpDados_Texto, tpNulo_Aceita) & _
      " AND PKID <> " & Formata_Dados(lngTIPOSOCIOID, tpDados_Longo, tpNulo_NaoAceita)
    Set objRs = objGer.ExecutarSQL(strSql)
    If Not objRs.EOF Then
      objRs.Close
      Set objRs = Nothing
      Set objGer = Nothing
      TratarErroPrevisto "Tipo Socio já cadastrado", "cmdOK_Click"
    ...
    ElseIf Status = tpStatus_Incluir Then
      ...
      '
      objTipoSocio.InserirTipoSocio txtDescricao.Text, _
                                    txtTipo.Text
      '
      bRetorno = True
    End If
...
Private Function ValidaCampos() As Boolean
  ...
  If Len(txtDescricao.Text) = 0 Then
    strMsg = strMsg & "Informar a descrição do tipo de sócio" & vbCrLf
    Pintar_Controle txtDescricao, tpCorContr_Erro
    SetarFoco txtDescricao
  End If
  If Len(txtTipo.Text) = 0 Then
    strMsg = strMsg & "Informar o tipo de sócio" & vbCrLf
    Pintar_Controle txtTipo, tpCorContr_Erro
    SetarFoco txtTipo
  End If
  '
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserTipoSocioInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserTipoSocioInc.Form_Activate]"
End Sub
...
On Error GoTo trata
  ...
  ElseIf Status = tpStatus_Alterar Or Status = tpStatus_Consultar Then
    ...
    Set objTipoSocio = New busApler.clsTipoSocio
    Set objRs = objTipoSocio.ListarTipoSocio(lngTIPOSOCIOID)
    '
    If Not objRs.EOF Then
