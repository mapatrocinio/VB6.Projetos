...
Begin VB.Form frmUserDocumentoInc 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Inclusão de documento"
   ClientHeight    =   2565
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10380
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      Tabs            =   1
      TabHeight       =   520
      TabCaption(0)   =   "&Dados do Documento"
      TabPicture(0)   =   "userDocumentoInc.frx":0000
   ...
   End
End
Attribute VB_Name = "frmUserDocumentoInc"
...
Private Sub cmdOK_Click()
  ...
  Case 0 'Inclusão/Alteração de Grupo cardápio
    ...
    strSql = "Select * From DOCUMENTO WHERE NOME = " & Formata_Dados(txtNome.Text, tpDados_Texto, tpNulo_Aceita) & _
      " AND PKID <> " & Formata_Dados(lngDOCUMENTOID, tpDados_Longo, tpNulo_NaoAceita)
    Set objRs = objGer.ExecutarSQL(strSql)
    If Not objRs.EOF Then
      objRs.Close
      Set objRs = Nothing
      Set objGer = Nothing
      TratarErroPrevisto "Documento já cadastrado", "cmdOK_Click"
    ...
    ElseIf Status = tpStatus_Incluir Then
      ...
      '
      objDocumento.InserirDocumento txtNome.Text
      '
      bRetorno = True
    End If
...
Private Function ValidaCampos() As Boolean
  ...
  If Len(txtNome.Text) = 0 Then
    strMsg = strMsg & "Informar o nome do documento" & vbCrLf
    Pintar_Controle txtNome, tpCorContr_Erro
    SetarFoco txtNome
  End If
  '
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserDocumentoInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserDocumentoInc.Form_Activate]"
End Sub
...
On Error GoTo trata
  ...
  ElseIf Status = tpStatus_Alterar Or Status = tpStatus_Consultar Then
    ...
    Set objDocumento = New busSisLoc.clsDocumento
    Set objRs = objDocumento.ListarDocumento(lngDOCUMENTOID)
    '
    If Not objRs.EOF Then
