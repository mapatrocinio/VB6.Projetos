...
Begin VB.Form frmUserDocumentoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Documento"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userDocumentoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserDocumentoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um documento!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserDocumentoInc.lngDOCUMENTOID = grdGeral.Columns("ID").Value
  frmUserDocumentoInc.Status = tpStatus_Alterar
  frmUserDocumentoInc.Show vbModal
  
  If frmUserDocumentoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  On Error GoTo trata
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um documento para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from ENTRADAMATERIAL where DOCUMENTOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Documento não pode ser excluido, pois já está associada a entrada de material.", "frmUserDocumentoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do documento " & grdGeral.Columns("Documento").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objDocumento.ExcluirDocumento CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserDocumentoInc.Status = tpStatus_Incluir
  frmUserDocumentoInc.lngDOCUMENTOID = 0
  frmUserDocumentoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT DOCUMENTO.PKID, DOCUMENTO.NOME " & _
            "FROM DOCUMENTO " & _
            "ORDER BY NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserDocumentoLis.grdGeral_UnboundReadDataEx]"
End Sub
