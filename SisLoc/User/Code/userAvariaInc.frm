...
Private Sub cboAvaria_Click()
  ...
  If objRs.EOF Then
    TratarErroPrevisto "Avaria " & cboAvaria.Text & " não cadastrada!"
    Status = tpStatus_Incluir
    lngAVARIAID = 0
    Form_Load
  Else
...
'''    Next
  ...
  Case 3 'Retirar Selecionados
    ...
    If blnSel = False Then
      TratarErroPrevisto "Nenhuma peça selecionada para exclusão.", "[frmUserAvariaInc.TratarAssociacao]"
    End If
  ...
  Case 5
    ...
    If strMsg = "" And grdNF.Columns(5) <> "" Then
      If CLng(grdNF.Columns(5)) = 0 Then
        strMsg = strMsg & "Quantidade lançada não pode ser zero" & vbCrLf
      End If
    End If
    If strMsg = "" And grdNF.Columns(5) <> "" Then
      If CLng(grdNF.Columns(5)) > CLng(grdNF.Columns(4)) Then
        strMsg = strMsg & "Quantidade não pode ser maior que a quantidade restante da peça na NF." & vbCrLf
      End If
    End If
  End Select
...
Private Function ValidaCamposItemNFDestinoAll() As Boolean
  ...
  If blnLancouItem = False Then
    blnRetorno = False
    TratarErroPrevisto "Nenhum item lançado para esta NF.", "[frmUserAvariaInc.ValidaCamposItemNFDestinoAll]"
  End If
  If blnRetorno = True Then
    'Nenhum erro encontrado
    If Not Valida_Data(mskData, TpObrigatorio, False) Then
      TratarErroPrevisto "Data inválida.", "[frmUserAvariaInc.ValidaCamposItemNFDestinoAll]"
      blnRetorno = False
    End If
  End If
