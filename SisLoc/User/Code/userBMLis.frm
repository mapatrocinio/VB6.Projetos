...
Begin VB.Form frmUserBMLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userBMLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   9090
   End
End
Attribute VB_Name = "frmUserBMLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um BM!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserBMInc.lngBMID = grdGeral.Columns("ID").Value
  frmUserBMInc.Status = tpStatus_Alterar
  frmUserBMInc.Show vbModal
  
  If frmUserBMInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  On Error GoTo trata
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um BM para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão o BM " & grdGeral.Columns("BM").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objBM.ExcluirBM CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdImprimir_Click()
  ...
  Report1.ReportFileName = gsReportPath & "BM.rpt"
  '
  ...
  '
  Report1.Formulas(0) = "BMID = " & grdGeral.Columns("ID").Value
  '
  Report1.Action = 1
  '
...
Private Sub cmdInserir_Click()
  ...
  frmUserBMInc.Status = tpStatus_Incluir
  frmUserBMInc.lngBMID = 0
  frmUserBMInc.Show vbModal
...
On Error GoTo trata
  ...
  LerFiguras Me, tpBmp_Vazio, , , cmdFechar, cmdExcluir, , cmdInserir, cmdAlterar, pbtnImprimir:=cmdImprimir
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT BM.PKID, EMPRESA.NOME, CONTRATO.NUMERO, BM.NUMERO, BM.MEDICAO, BM.DATAINICIO, BM.DATATERMINO " & _
            "FROM BM " & _
            " INNER JOIN CONTRATO ON CONTRATO.PKID = BM.CONTRATOID " & _
            " INNER JOIN EMPRESA ON EMPRESA.PKID = CONTRATO.EMPRESAID " & _
            "ORDER BY EMPRESA.NOME, CONTRATO.NUMERO, BM.NUMERO DESC"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBMLis.grdGeral_UnboundReadDataEx]"
End Sub
