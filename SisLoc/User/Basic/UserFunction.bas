...
'Propósito: Retornar e Gravar o Sequencial
Public Function RetornaGravaCampoSequencialBM(strCampo As String, _
                                              strContrato As String) As Long
  On Error GoTo trata
  '
  ...
  strSql = "SELECT ISNULL(MAX(" & strCampo & "), 0) + 1 AS SEQ " & _
    "From SEQUENCIALBMCTR " & _
    " WHERE CONTRATO = " & Formata_Dados(strContrato, tpDados_Texto)
  ...
  '
  If lngSeq = 1 Then
    strSql = "INSERT INTO SEQUENCIALBMCTR (" & strCampo & ", CONTRATO) VALUES (" & _
        Formata_Dados(lngSeq, tpDados_Longo) & _
        ", " & Formata_Dados(strContrato, tpDados_Texto) & _
        ")"
  Else
    strSql = "UPDATE SEQUENCIALBMCTR SET " & strCampo & " = " & _
      Formata_Dados(lngSeq, tpDados_Longo) & _
      " WHERE CONTRATO = " & Formata_Dados(strContrato, tpDados_Texto)
  End If
  '---
  ...
  '
  RetornaGravaCampoSequencialBM = CLng(lngSeq)
  '
  Set objGeral = Nothing
  Exit Function
trata:
  Err.Raise Err.Number, "[mdlUserFunction.RetornaGravaCampoSequencialBM]", Err.Description
End Function

'Propósito: Retornar e Gravar Medicao do BM
Public Function RetornaMedicaoBM(lngCONTRATOID As Long) As String
  On Error GoTo trata
  '
  Dim objRs       As ADODB.Recordset
  Dim strSql      As String
  ...
  strSql = "SELECT COUNT(PKID)AS TOTAL FROM BM " & _
      "WHERE CONTRATOID = " & Formata_Dados(lngCONTRATOID, tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  '
  If objRs.EOF Then
    strStatus = "I"
  ElseIf objRs.Fields("TOTAL").Value = 0 Then
    strStatus = "I"
  End If
  objRs.Close
  Set objRs = Nothing
  '
  If strStatus = "" Then
    'Não está no estado inicial
    'Verifica se ainda possui NF a lançar
    strSql = "SELECT COUNT(PKID)AS TOTAL FROM NF " & _
            "WHERE STATUS IN ('I', 'F') " & _
            " AND CONTRATOID = " & Formata_Dados(lngCONTRATOID, tpDados_Longo)
    Set objRs = objGeral.ExecutarSQL(strSql)
    '
    If objRs.EOF Then
      strStatus = "F"
  ...
  End If
  '
  RetornaMedicaoBM = strStatus
  '
  Set objGeral = Nothing
  Exit Function
trata:
  Err.Raise Err.Number, _
            "[mdlUserFunction.RetornaMedicaoBM]", _
            Err.Description
End Function
...
Public Function RetornaGravaCampoSequencialCtrto(strCampo As String, _
                                                 lngAno As Long) As Long
  On Error GoTo trata
  '
  Dim objGeral        As busSisLoc.clsGeral
  Dim objRs           As ADODB.Recordset
  ...
  Set objGeral = New busSisLoc.clsGeral
  strSql = "SELECT ISNULL(MAX(" & strCampo & "), 0) + 1 AS SEQ " & _
    "From SEQUENCIALCTRTOANO " & _
    " WHERE ANO = " & Formata_Dados(lngAno, tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  '
  If objRs.EOF Then
    lngSeq = 1
  ...
  '
  If lngSeq = 1 Then
    strSql = "INSERT INTO SEQUENCIALCTRTOANO (" & strCampo & ", ANO) VALUES (" & _
        Formata_Dados(lngSeq, tpDados_Longo) & _
        ", " & Formata_Dados(lngAno, tpDados_Longo) & _
        ")"
  Else
    strSql = "UPDATE SEQUENCIALCTRTOANO SET " & strCampo & " = " & _
      Formata_Dados(lngSeq, tpDados_Longo) & _
      " WHERE ANO = " & Formata_Dados(lngAno, tpDados_Longo)
  End If
  '---
  objGeral.ExecutarSQLAtualizacao (strSql)
  '
  RetornaGravaCampoSequencialCtrto = CLng(lngSeq)
  '
  Set objGeral = Nothing
  Exit Function
trata:
  Err.Raise Err.Number, "[mdlUserFunction.RetornaGravaCampoSequencialCtrto]", Err.Description
End Function

'Propósito: Retornar e Gravar o Sequencial (senha)
Public Function RetornaGravaCampoSequencialNF(strCampo As String, _
                                              lngAno As Long) As Long
  On Error GoTo trata
  '
  Dim objGeral        As busSisLoc.clsGeral
  ...
  '
  If lngSeq = 1 Then
    strSql = "INSERT INTO SEQUENCIALNFANO (" & strCampo & ", ANO) VALUES (" & _
        Formata_Dados(lngSeq, tpDados_Longo) & _
        ", " & Formata_Dados(lngAno, tpDados_Longo) & _
        ")"
  Else
    strSql = "UPDATE SEQUENCIALNFANO SET " & strCampo & " = " & _
      Formata_Dados(lngSeq, tpDados_Longo) & _
      " WHERE ANO = " & Formata_Dados(lngAno, tpDados_Longo)
  End If
  '---
  objGeral.ExecutarSQLAtualizacao (strSql)
  '
  RetornaGravaCampoSequencialNF = CLng(lngSeq)
  '
  Set objGeral = Nothing
  Exit Function
trata:
  Err.Raise Err.Number, "[mdlUserFunction.RetornaGravaCampoSequencialNF]", Err.Description
End Function
...
Public Sub TratarErro(ByVal pNumero As Long, _
  ...
  intI = FreeFile
  Open App.Path & "\SisLoc.txt" For Append As #intI
  ...
  MsgBox "O Seginte Erro Ocorreu: " & vbCrLf & vbCrLf & _
    ...
    "Erro gravado no arquivo: " & App.Path & "\SisLoc.txt" & vbCrLf & vbCrLf & _
...
Public Sub VerificaMovAposFecha(lngNFID As Long)
  On Error GoTo trata
  ...
  Dim vrTotNF         As Currency
  Dim objNF           As busSisLoc.clsNF
  Dim strStatus       As String
  '
  Set objGeral = New busSisLoc.clsGeral
  '
  'Capturar valor total da NF
  strSql = "SELECT NF.STATUS " & _
    "FROM NF " & _
    "WHERE PKID = " & Formata_Dados(lngNFID, tpDados_Longo)

  Set objRs = objGeral.ExecutarSQL(strSql)
  strStatus = ""
  If Not objRs.EOF Then
    If Not IsNull(objRs.Fields("STATUS").Value) Then
      strStatus = objRs.Fields("STATUS").Value
    End If
  End If
  objRs.Close
  Set objRs = Nothing
  'Sai se NF ainda não estiver fechada
  If strStatus <> "F" And strStatus <> "S" Then Exit Sub
...
'Proposito: Verificar consolidação da NF
Public Sub VerificaStatusConsolicacao(lngNFID As Long)
  On Error GoTo trata
  'Retorna 0 - para Código de Erro
  Dim strSql        As String
  Dim objRs         As ADODB.Recordset
  ...
  '
  'Capturar valor total da NF
  strSql = "SELECT ITEMNF.PKID, " & _
      " MIN(ITEMNF.QUANTIDADE) - (ISNULL(SUM(DEVOL.QTD_DEV), 0)) - (ISNULL(SUM(DEVOL.QTD_AVARIA), 0)) AS QTD_REST " & _
      " From ITEMNF " & _
      " LEFT JOIN (SELECT ITEMDEVOLUCAO.ITEMNFID, ISNULL(SUM(ITEMDEVOLUCAO.QUANTIDADE), 0) AS QTD_DEV, ISNULL(SUM(ITEMDEVOLUCAO.QUANTIDADEAVARIA), 0) AS QTD_AVARIA " & _
      " From ITEMDEVOLUCAO " & _
      " GROUP BY ITEMDEVOLUCAO.ITEMNFID) AS DEVOL ON ITEMNF.PKID = DEVOL.ITEMNFID " & _
      " WHERE NFID = " & Formata_Dados(lngNFID, tpDados_Longo) & _
      " GROUP BY ITEMNF.PKID " & _
      " HAVING MIN(ITEMNF.QUANTIDADE) - (ISNULL(SUM(DEVOL.QTD_DEV), 0)) - (ISNULL(SUM(DEVOL.QTD_AVARIA), 0)) > 0;"

  Set objRs = objGeral.ExecutarSQL(strSql)
  strStatus = ""
  If objRs.EOF Then
    'NÃO HÁ MAIS ITENS A SEREM LANÇADOS, CONSOLIDA NF
    strStatus = "S"
  Else
    strStatus = "F"
  End If
