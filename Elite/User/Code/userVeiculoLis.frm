...
Begin VB.Form frmVeiculoLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userVeiculoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   7380
   End
End
Attribute VB_Name = "frmVeiculoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value & "") Then
    MsgBox "Selecione um veículo!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If

  frmVeiculoInc.lngVEICULOID = grdGeral.Columns("ID").Value
  frmVeiculoInc.Status = tpStatus_Alterar
  frmVeiculoInc.Show vbModal

  If frmVeiculoInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um veículo para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do veículo de placa " & grdGeral.Columns("Placa").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objGeral = New busElite.clsGeral
  strSql = "SELECT * FROM PACOTE WHERE VEICULOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Veículo não pode ser excluído por constar referências na tabela de [PACOTE]", "frmUserVeiculoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  objVeiculo.ExcluirVeiculo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmVeiculoInc.Status = tpStatus_Incluir
  frmVeiculoInc.lngVEICULOID = 0
  frmVeiculoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  Set objGeral = New busElite.clsGeral
  '
  strSql = "SELECT VEICULO.PKID, MARCA.NOME, MODELO.NOME, VEICULO.PLACA " & _
            "FROM VEICULO " & _
            " LEFT JOIN MODELO ON MODELO.PKID = VEICULO.MODELOID " & _
            " LEFT JOIN MARCA ON MARCA.PKID = MODELO.MARCAID " & _
            "ORDER BY MARCA.NOME, MODELO.NOME, VEICULO.PLACA"
  '
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmVeiculoLis.grdGeral_UnboundReadDataEx]"
End Sub
