...
Begin VB.Form frmServicoAssoc 
   ...
   Caption         =   "Associação de Serviços ao Pacote"
   ClientHeight    =   7245
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11520
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      TabsPerRow      =   1
      TabHeight       =   520
      TabCaption(0)   =   "Associar Serviços"
      ...
      End
      Begin VB.Label Label1 
         Caption         =   "Serviços não associados"
         Height          =   195
         Index           =   17
         Left            =   150
         TabIndex        =   10
...
Private Sub TratarAssociacao(pIndice As Integer)
    ...
    For intI = 0 To grdUnidadeAssoc.SelBookmarks.Count - 1
      ...
      If grdUnidadeAssoc.Columns("Status").Text & "" = "FINALIZADO" Then
        strMsgErroExc = strMsgErroExc & grdUnidadeAssoc.Columns("Data/hora").Text & _
                grdUnidadeAssoc.Columns("Agência").Text & _
  ...
  If strMsgErroExc <> "" Then
    strMsgErroExc = "O(s) serviço(s) abaxo não pode(m) ser excluido(s) pois já forma finalizados: " & _
