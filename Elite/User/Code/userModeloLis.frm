...
Begin VB.Form frmModeloLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userModeloLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   7380
   End
End
Attribute VB_Name = "frmModeloLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value & "") Then
    MsgBox "Selecione um modelo!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If

  frmModeloInc.lngMODELOID = grdGeral.Columns("ID").Value
  frmModeloInc.Status = tpStatus_Alterar
  frmModeloInc.Show vbModal

  If frmModeloInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um modelo para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do modelo " & grdGeral.Columns("Modelo").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objGeral = New busElite.clsGeral
  strSql = "SELECT * FROM VEICULO WHERE MODELOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Modelo não pode ser excluído por constar referências na tabela de [VEÍCULOS]", "frmUserModeloLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  objModelo.ExcluirModelo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmModeloInc.Status = tpStatus_Incluir
  frmModeloInc.lngMODELOID = 0
  frmModeloInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  Set objGeral = New busElite.clsGeral
  '
  strSql = "SELECT MODELO.PKID, MARCA.NOME, MODELO.NOME " & _
            "FROM MODELO " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmModeloLis.grdGeral_UnboundReadDataEx]"
End Sub
