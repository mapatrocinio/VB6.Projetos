...
Begin VB.Form frmAgenciaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de agências"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10080
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userAgenciaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8160
   End
End
Attribute VB_Name = "frmAgenciaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma agência!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmAgenciaInc.Status = tpStatus_Alterar
  frmAgenciaInc.lngPKID = grdGeral.Columns("PKID").Value
  frmAgenciaInc.Show vbModal
  
  If frmAgenciaInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma agência para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da agência " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objAgencia = New busElite.clsAgencia
  objAgencia.ExcluirAgencia CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmAgenciaInc.Status = tpStatus_Incluir
  frmAgenciaInc.lngPKID = 0
  frmAgenciaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de Agências
  strSql = "SELECT AGENCIA.PKID, AGENCIA.NOME "
  strSql = strSql & " FROM AGENCIA " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmAgencia.grdGeral_UnboundReadDataEx]"
End Sub
