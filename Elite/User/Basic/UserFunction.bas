...
Public Sub TratarErro(ByVal pNumero As Long, _
  ...
  intI = FreeFile
  Open App.Path & "\Elite.txt" For Append As #intI
  ...
  MsgBox "O Seginte Erro Ocorreu: " & vbCrLf & vbCrLf & _
    ...
    "Erro gravado no arquivo: " & App.Path & "\Elite.txt" & vbCrLf & vbCrLf & _
...
Public Function RetornaCPFSemMascara(CPF As String) As String
  'Recebe o CPF e retorna o CPF sem máscara
  On Error GoTo trata
  Dim strRetorno As String
  
  strRetorno = Left(CPF, 2) & Mid(CPF, 4, 3) & Mid(CPF, 8, 3) & Mid(CPF, 12, 4) & Right(CPF, 2)
  RetornaCPFSemMascara = strRetorno
  Exit Function
trata:
  Err.Raise Err.Number, _
            "[mdlUserFunction.RetornaCPFSemMascara]", _
            Err.Description
End Function
