...
Begin VB.Form frmUserRelReceitas 
   Caption         =   "Demonstrativo de Receitas"
   ClientHeight    =   4665
   ClientLeft      =   450
   ClientTop       =   1920
   ClientWidth     =   6105
   ...
   End
End
Attribute VB_Name = "frmUserRelReceitas"
...
Private Sub cmdRelatorio_Click()
  ...
  If optConta(0).Value Then
    'Contas Pagas
    Report1.ReportFileName = gsReportPath & "DemoReceitasRecebidas.rpt"
  Else
    'Contas a Pagar
    Report1.ReportFileName = gsReportPath & "DemoReceitasAReceber.rpt"
  End If
  ...
  If optConta(0).Value Then
    If cboTipo.Text = "" Or cboTipo.Text = "<TODOS>" Then
      Report1.Formulas(6) = "DespesaTipo = True = true"
    Else
      Report1.Formulas(6) = "DespesaTipo = {DESPESA.TIPO} = '" & IIf(cboTipo.Text = "Administração", "A", "T") & "'"
    End If
  End If
