...
Begin VB.Form frmUserContaLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userContaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8280
   End
End
Attribute VB_Name = "frmUserContaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma conta !", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmUserContaInc.Status = tpStatus_Alterar
  frmUserContaInc.lngCONTAID = grdGeral.Columns("ID").Value
  frmUserContaInc.Show vbModal
  
  If frmUserContaInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    MsgBox "Selecione uma Conta para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da Conta " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objGeral = New busSisContas.clsGeral
  strSql = "SELECT * FROM MOVIMENTACAO WHERE CONTADEBITOID = " & CLng(grdGeral.Columns("ID").Value) & _
    " OR CONTACREDITOID = " & CLng(grdGeral.Columns("ID").Value)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Não é possível excluir a conta, por constar movimentações cadastradas para ela.", "[cmdExcluir_Click]"
    Exit Sub
  End If
  objRs.Close
  Set objRs = Nothing
  ...
  objConta.ExcluirConta CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserContaInc.Status = tpStatus_Incluir
  frmUserContaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT CONTA.PKID, CONTA.DESCRICAO, TIPOCONTA.DESCRICAO, CONTA.DTSALDO, CONTA.VRSALDO " & _
            "FROM CONTA LEFT JOIN TIPOCONTA ON TIPOCONTA.PKID = CONTA.TIPOCONTAID " & _
            " AND PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
