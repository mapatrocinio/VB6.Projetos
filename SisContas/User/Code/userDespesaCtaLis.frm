...
Begin VB.Form frmUserDespesaCtaLis 
   ...
   Begin VB.PictureBox picBotoes 
      ...
      Begin VB.PictureBox SSPanel1 
         ...
         End
      End
      Begin VB.Label lblTituloConfiguracao 
         Alignment       =   2  'Center
         Caption         =   "Label1"
         BeginProperty Font 
            Name            =   "Arial"
   ...
   End
End
Attribute VB_Name = "frmUserDespesaCtaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Trim(grdGeral.Columns("TIPO").Value & "") = "T" Then
    frmUserDespesaCtaInc.Status = tpStatus_Alterar
  Else
    frmUserDespesaCtaInc.Status = tpStatus_Alterar
  End If
  frmUserDespesaCtaInc.lngDESPESAID = grdGeral.Columns("ID").Value
  frmUserDespesaCtaInc.strTipo = Trim(grdGeral.Columns("TIPO").Value & "")
  frmUserDespesaCtaInc.strTipoCtaPagas = strTipoCtaPagas
  frmUserDespesaCtaInc.Show vbModal
  
  If frmUserDespesaCtaInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objDespesa As busSisContas.clsDespesaCta
  '
  On Error GoTo trata
  ...
  End If
  If Trim(grdGeral.Columns("TIPO").Value & "") = "T" Then
    MsgBox "Esta despesa foi criada pelo SisContas, não poderá ser excluida.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da despesa " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objDespesa = New busSisContas.clsDespesaCta
...
Private Sub cmdInserir_Click()
  ...
  frmUserDespesaCtaInc.Status = tpStatus_Incluir
  frmUserDespesaCtaInc.strTipo = "A" 'Administração
  frmUserDespesaCtaInc.strTipoCtaPagas = strTipoCtaPagas
  frmUserDespesaCtaInc.Show vbModal
...
On Error GoTo trata
  ...
  lblTituloConfiguracao.Caption = RetornaTituloConfiguracao
...
Public Sub MontaMatriz()
  ...
  If strTipoCtaPagas = "N" Then
    strSql = "SELECT DESPESA.PKID, DESPESA.TIPO, DESPESA.SEQUENCIAL, case DESPESA.TIPO when 'T' then 'Telefonista' when 'A' then 'Administração' else '' end, convert(varchar(20), GRUPODESPESA.CODIGO) + convert(varchar(20), SUBGRUPODESPESA.CODIGO) + ' - ' + SUBGRUPODESPESA.DESCRICAO, DESPESA.DESCRICAO, DESPESA.VR_PAGO " & _
              ...
              " WHERE DESPESA.PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
              IIf(strTipo = "A", " AND DESPESA.TIPO='A'", "")
  Else
    strSql = "SELECT DESPESA.PKID, DESPESA.TIPO, DESPESA.SEQUENCIAL, case DESPESA.TIPO when 'T' then 'Telefonista' when 'A' then 'Administração' else '' end, convert(varchar(20), GRUPODESPESA.CODIGO) + convert(varchar(20), SUBGRUPODESPESA.CODIGO) + ' - ' + SUBGRUPODESPESA.DESCRICAO, DESPESA.DESCRICAO, DESPESA.VR_PAGO " & _
              "FROM DESPESA LEFT JOIN SUBGRUPODESPESA ON SUBGRUPODESPESA.PKID = DESPESA.SUBGRUPODESPESAID " & _
              " LEFT JOIN GRUPODESPESA ON GRUPODESPESA.PKID = SUBGRUPODESPESA.GRUPODESPESAID " & _
              " WHERE DESPESA.DT_PAGAMENTO is null " & _
              " AND DESPESA.PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
              IIf(strTipo = "A", " AND DESPESA.TIPO='A'", "")
  End If
