...
Begin VB.Form frmUserGrupoDespesaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de grupos de despesa"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8820
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userGrupoDespesaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6705
   End
End
Attribute VB_Name = "frmUserGrupoDespesaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um grupo de despesa !", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmUserGrupoDespesaInc.Status = tpStatus_Alterar
  frmUserGrupoDespesaInc.lngGRUPODESPESAID = grdGeral.Columns("ID").Value
  frmUserGrupoDespesaInc.Show vbModal
  
  If frmUserGrupoDespesaInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objGrupoDespesa   As busSisContas.clsGrupoDespesa
  ...
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    ...
    Exit Sub
  End If
  If MsgBox("Confirma exclusão do grupo de despesa " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objGeral = New busSisContas.clsGeral
  strSql = "SELECT * FROM SUBGRUPODESPESA WHERE GRUPODESPESAID = " & CLng(grdGeral.Columns("ID").Value)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Não é possível excluir o grupo despesa, por constar sub grupos cadastrados para ele.", "[cmdExcluir_Click]"
    Exit Sub
  End If
  objRs.Close
  Set objRs = Nothing
  Set objGeral = Nothing
  Set objGrupoDespesa = New busSisContas.clsGrupoDespesa
  
  objGrupoDespesa.ExcluirGrupoDespesa CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserGrupoDespesaInc.Status = tpStatus_Incluir
  frmUserGrupoDespesaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT GRUPODESPESA.PKID, GRUPODESPESA.CODIGO, GRUPODESPESA.DESCRICAO, case GRUPODESPESA.TIPO when 'D' then 'DÉBITO' when 'C' then 'CRÉDITO' else '' end FROM GRUPODESPESA " & _
