...
Begin VB.Form frmGrupoProdutoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Grupo Produto"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8550
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userGrupoProdutoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6480
   End
End
Attribute VB_Name = "frmGrupoProdutoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um grupo de produto!", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmGrupoProdutoInc.Status = tpStatus_Alterar
  frmGrupoProdutoInc.lngGRUPO_PRODUTOID = grdGeral.Columns("ID").Value
  frmGrupoProdutoInc.Show vbModal
  
  If frmGrupoProdutoInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  On Error GoTo trata
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um Grupo Produto para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  Set objGer = New busSisMetal.clsGeral
  'ACESSÓRIO
  strSql = "Select * from PRODUTO WHERE GRUPO_PRODUTOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Grupo Produto não pode ser excluido, pois já está associado a produto.", "frmGrupoProdutoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  '
  If MsgBox("Confirma exclusão da Grupo Produto " & grdGeral.Columns("Nome do grupo").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objGrupoProduto = New busSisMetal.clsGrupoProduto
  
  objGrupoProduto.ExcluirGrupoProduto CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmGrupoProdutoInc.Status = tpStatus_Incluir
  frmGrupoProdutoInc.lngGRUPO_PRODUTOID = 0
  frmGrupoProdutoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT GRUPO_PRODUTO.PKID, GRUPO_PRODUTO.NOME " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmGrupoProdutoLis.grdGeral_UnboundReadDataEx]"
End Sub
