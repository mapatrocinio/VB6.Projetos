...
Begin VB.Form frmGrupoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Grupo"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8550
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userGrupoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6480
   End
End
Attribute VB_Name = "frmGrupoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um grupo!", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmGrupoInc.Status = tpStatus_Alterar
  frmGrupoInc.lngGRUPOID = grdGeral.Columns("ID").Value
  frmGrupoInc.Show vbModal
  
  If frmGrupoInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  On Error GoTo trata
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um Grupo para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  Set objGer = New busSisMetal.clsGeral
  'ACESSÓRIO
  strSql = "Select * from ACESSORIO WHERE GRUPOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Grupo não pode ser excluido, pois já está associado a acessório.", "frmGrupoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  '
  If MsgBox("Confirma exclusão da Grupo " & grdGeral.Columns("Nome do grupo").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objGrupo = New busSisMetal.clsGrupo
  
  objGrupo.ExcluirGrupo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmGrupoInc.Status = tpStatus_Incluir
  frmGrupoInc.lngGRUPOID = 0
  frmGrupoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT GRUPO.PKID, GRUPO.NOME " & _
            "FROM GRUPO " & _
            " ORDER BY GRUPO.NOME "
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmGrupoLis.grdGeral_UnboundReadDataEx]"
End Sub
