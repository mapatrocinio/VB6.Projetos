...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    ...
    Exit Sub
  End If
  frmLivroInc.Status = tpStatus_Alterar
  frmLivroInc.lngLIVROID = grdGeral.Columns("ID").Value
  frmLivroInc.Show vbModal
  
  If frmLivroInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objLivro As busSisMetal.clsLivro
  '
  On Error GoTo trata
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    ...
    Exit Sub
  End If
  If MsgBox("Confirma exclusão do livro " & grdGeral.Columns("Nro. Livro").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objLivro = New busSisMetal.clsLivro
...
Private Sub cmdInserir_Click()
  ...
  frmLivroInc.Status = tpStatus_Incluir
  frmLivroInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT LIVRO.PKID, LIVRO.NUMEROLIVRO, BANCO.NOME, LIVRO.AGENCIA, LIVRO.CONTA FROM LIVRO " & _
    " LEFT JOIN BANCO ON BANCO.PKID = LIVRO.BANCOID "
