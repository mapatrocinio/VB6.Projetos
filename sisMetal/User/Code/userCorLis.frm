...
Begin VB.Form frmCorLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Cor"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8550
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userCorLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6480
   End
End
Attribute VB_Name = "frmCorLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma cor!", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmCorInc.Status = tpStatus_Alterar
  frmCorInc.lngCORID = grdGeral.Columns("ID").Value
  frmCorInc.Show vbModal
  
  If frmCorInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objCor       As busSisMetal.clsCor
  ...
  On Error GoTo trata
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma Cor para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  Set objGer = New busSisMetal.clsGeral
  '
  strSql = "Select * from PERFIL WHERE CORID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Cor não pode ser excluida, pois já está associada a perfis.", "frmCorLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  '
  If MsgBox("Confirma exclusão do Cor " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objCor = New busSisMetal.clsCor
  
  objCor.ExcluirCor CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmCorInc.Status = tpStatus_Incluir
  frmCorInc.lngCORID = 0
  frmCorInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT COR.PKID, COR.NOME, COR.SIGLA " & _
            "FROM COR " & _
            " ORDER BY COR.NOME "
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmCorLis.grdGeral_UnboundReadDataEx]"
End Sub
