...
Begin VB.Form frmGerencialRec 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Gerencial de Recebimento"
   ClientHeight    =   8715
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12000
   ...
   Begin VB.Frame fraUnidade 
      ...
      Begin TrueDBGrid60.TDBGrid grdGeral 
         Height          =   5730
         Left            =   90
         OleObjectBlob   =   "userGerencialRec.frx":0000
         TabIndex        =   0
         Top             =   180
         Width           =   11580
      End
   ...
   Begin VB.Frame fraImpressao 
      ...
      Begin VB.Label Label72 
         Caption         =   "CTRL + B - Fechamento Caixa"
         BeginProperty Font 
            Name            =   "Small Fonts"
            Size            =   6.75
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmGerencialRec"
...
'''    '
  Case 2
    ...
    If gsNivel = "CAI" Or gsNivel = "ADM" Then
      'Só pede senha superior se quem estiver logado não for superior
      frmLoginSup.Show vbModal
      
      If Len(Trim(gsNomeUsuLib)) = 0 Then
        strMsg = "É necessário de logar para impressão do fechamento do caixa."
        TratarErroPrevisto strMsg, "cmdConfirmar_Click"
        Exit Sub
      ElseIf gsNivelUsuLib <> "CAI" Then
        strMsg = "Apenas o perfil Caixa-Recebimento pode realizar a impressão do fechamento do caixa."
        TratarErroPrevisto strMsg, "cmdConfirmar_Click"
        Exit Sub
    ...
    Else
      MsgBox "Apenas o perfil Caixa-Recebimento pode realizar a impressão do fechamento do caixa.", vbExclamation, TITULOSISTEMA
      SetarFoco grdGeral
      Exit Sub
    End If
    '--------------------------------
    '--------------------------------
    IMP_COMPROV_FECHA_TURNO giFunIdUsuLib, Format(Now, "DD/MM/YYYY"), gsNomeEmpresa
    '
  End Select
  Exit Sub
trata:
...
Public Sub VerificaQuemChamou()
  ...
  Case 0
    ...
    If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
      MsgBox "Selecione um Pedido para efetuar o recebimento.", vbExclamation, TITULOSISTEMA
      SetarFoco grdGeral
      Exit Sub
    End If
    'Verifica status
    ...
    If Trim(grdGeral.Columns("Status").Value & "") = "X" Then
      ...
      Exit Sub
    End If
    If Trim(grdGeral.Columns("Status").Value & "") = "R" Then
      MsgBox "Pedido com recebimento.", vbExclamation, TITULOSISTEMA
      SetarFoco grdGeral
      Exit Sub
    End If
    '----------------------------
    ...
    If gsNivel = "CAI" Or gsNivel = "ADM" Then
      ...
      If Len(Trim(gsNomeUsuLib)) = 0 Then
        strMsg = "É necessário de logar para realizar recebimentos."
        TratarErroPrevisto strMsg, "cmdConfirmar_Click"
        Exit Sub
      ElseIf gsNivelUsuLib <> "CAI" Then
        strMsg = "Apenas o perfil Caixa-Recebimento pode realizar recebimentos."
        TratarErroPrevisto strMsg, "cmdConfirmar_Click"
        Exit Sub
      ...
      'Capturou Nome do Usuário, continua processo de Venda
    Else
      MsgBox "Apenas o perfil Caixa-Recebimento pode realizar recebimentos.", vbExclamation, TITULOSISTEMA
      SetarFoco grdGeral
      Exit Sub
    End If
    '--------------------------------
    '--------------------------------
    frmRecebimentoPedido.Status = tpStatus_Incluir
    frmRecebimentoPedido.lngPEDIDOVENDAID = grdGeral.Columns("ID").Value
    frmRecebimentoPedido.Show vbModal
  Case 3
...
Public Sub MontaMatriz()
  ...
  On Error GoTo trata
  '
  strDataCanc = Format(DateAdd("d", (giQtdDiasPedido * (-1)), Now), "DD/MM/YYYY")
  ...
  strSql = "SELECT PEDIDOVENDA.PKID, PEDIDOVENDA.PED_NUMERO , PEDIDOVENDA.DATA, MIN(PESSOA.NOME), CASE MIN(TIPOVENDA.DESCRICAO) WHEN 'EMPRESA' THEN MIN(LOJA.NOME) WHEN 'CLIENTE' THEN MIN(FICHACLIENTE.NOME) ELSE '' END, SUM(ITEM_PEDIDOVENDA.QUANTIDADE), SUM(ITEM_PEDIDOVENDA.VALOR), " & _
        ...
        " AND PEDIDOVENDA.STATUS IN ('B', 'L', 'E', 'R') " & _
        " GROUP BY PEDIDOVENDA.PKID, PEDIDOVENDA.PED_NUMERO , PEDIDOVENDA.DATA " & _
        " ORDER BY YEAR(PEDIDOVENDA.DATA) DESC, MONTH(PEDIDOVENDA.DATA) DESC, DAY(PEDIDOVENDA.DATA) DESC, PEDIDOVENDA.PED_NUMERO DESC;"
  ''B-BALCÃO, L-CLIENTE, E-EMPRESA NÃO MOSTRA CANCELADO E EXCLUIDO
  'TAMBÉM EXIBE O QUE ESTÁ RECEBIDO R-RECEBIMENTO
