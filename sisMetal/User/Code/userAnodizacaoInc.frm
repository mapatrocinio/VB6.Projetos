...
Begin VB.Form frmAnodizacaoInc 
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      TabPicture(0)   =   "userAnodizacaoInc.frx":0000
      ...
      Begin TrueDBGrid60.TDBGrid grdOS 
         Height          =   4890
         Left            =   90
         OleObjectBlob   =   "userAnodizacaoInc.frx":001C
         TabIndex        =   0
         Top             =   780
         Width           =   9210
      End
      Begin VB.Label Label1 
         Caption         =   $"userAnodizacaoInc.frx":6F7B
         ForeColor       =   &H000000FF&
         Height          =   435
         Index           =   2
         Left            =   120
   ...
   End
End
Attribute VB_Name = "frmAnodizacaoInc"
...
Public Sub ANOD_MontaMatriz()
  ...
  strSql = "SELECT VW_CONS_ITEM_OS.ITEM_OSID, VW_CONS_ITEM_OS.LINHAID,'N',VW_CONS_ITEM_OS.NOME, VW_CONS_ITEM_OS.CODIGO, VW_CONS_ITEM_OS.QUANTIDADE, VW_CONS_ITEM_OS.ANOD_BRA_QUANTIDADE, VW_CONS_ITEM_OS.ANOD_BRI_QUANTIDADE, VW_CONS_ITEM_OS.ANOD_BRO_QUANTIDADE, VW_CONS_ITEM_OS.ANOD_NAT_QUANTIDADE " & _
           ...
           " WHERE VW_CONS_ITEM_OS.OSID = " & Formata_Dados(lngOSID, tpDados_Longo) & _
...
Private Sub cmdOK_Click()
  Dim objAnodizacao As busSisMetal.clsAnodizacao
  On Error GoTo trata
  Dim intI      As Integer
  '
  ...
  Case 0 'Gravar Anodização
    ...
    '
    Set objAnodizacao = New busSisMetal.clsAnodizacao
    For intI = 0 To ANOD_LINHASMATRIZ - 1
      ...
      If grdOS.Columns("*").Text & "" = "<Bitmap>.S" Then
        ...
        objAnodizacao.InserirAnodizacaoItem lngOSID, _
                                            grdOS.Columns("PKID").Text & "", _
                                            grdOS.Columns("LINHAID").Text & "", _
                                            grdOS.Columns("Qtd.").Text & "", _
                                            grdOS.Columns("Branco").Text & "", _
                                            grdOS.Columns("Brilho").Text & "", _
                                            grdOS.Columns("Bronze").Text & "", _
                                            grdOS.Columns("Natural").Text
        blnRetorno = True
      End If
    Next
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmAnodizacaoInc.Form_Activate]"
End Sub
...
Private Sub grdOS_BeforeUpdate(Cancel As Integer)
  On Error GoTo trata
  'Atualiza Matriz
  ANOD_Matriz(2, grdOS.Row) = "S"
  ANOD_Matriz(6, grdOS.Row) = grdOS.Columns(6).Text
  ANOD_Matriz(7, grdOS.Row) = grdOS.Columns(7).Text
  ANOD_Matriz(8, grdOS.Row) = grdOS.Columns(8).Text
  ANOD_Matriz(9, grdOS.Row) = grdOS.Columns(9).Text
  '
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmAnodizacaoInc.grdOS_BeforeRowColChange]"
End Sub
...
Private Function ValidaCamposAnodOrigemLinha(intLinha As Integer) As Boolean
  ...
  Dim lngTotalBRA           As Long
  Dim lngTotalBRI           As Long
  Dim lngTotalBRO           As Long
  Dim lngTotalNAT           As Long
  '
  blnSetarFocoControle = True
  '
  strMsg = ""
  'Validção da anodização
  If Not Valida_Moeda(grdOS.Columns("Branco"), TpnaoObrigatorio, blnSetarFocoControle, blnPintarControle:=False, blnValidarPeloClip:=False) Then
    strMsg = strMsg & "Quantidade de anodização BRANCO inválida na linha " & intLinha + 1 & vbCrLf
  End If
  If Not Valida_Moeda(grdOS.Columns("Brilho"), TpnaoObrigatorio, blnSetarFocoControle, blnPintarControle:=False, blnValidarPeloClip:=False) Then
    strMsg = strMsg & "Quantidade de anodização BRILHO inválida na linha " & intLinha + 1 & vbCrLf
  End If
  If Not Valida_Moeda(grdOS.Columns("Bronze"), TpnaoObrigatorio, blnSetarFocoControle, blnPintarControle:=False, blnValidarPeloClip:=False) Then
    strMsg = strMsg & "Quantidade de anodização BRONZE inválida na linha " & intLinha + 1 & vbCrLf
  End If
  If Not Valida_Moeda(grdOS.Columns("Natural"), TpnaoObrigatorio, blnSetarFocoControle, blnPintarControle:=False, blnValidarPeloClip:=False) Then
    strMsg = strMsg & "Quantidade de anodização NATURAL inválida na linha " & intLinha + 1 & vbCrLf
  End If
  '
  If Len(strMsg) = 0 Then
    ...
    lngTotalNAT = 0
    '
    lngTotal = CLng(grdOS.Columns("Qtd."))
    lngTotalBRA = CLng(IIf(Not IsNumeric(grdOS.Columns("Branco")), 0, grdOS.Columns("Branco")))
    lngTotalBRI = CLng(IIf(Not IsNumeric(grdOS.Columns("Brilho")), 0, grdOS.Columns("Brilho")))
    lngTotalBRO = CLng(IIf(Not IsNumeric(grdOS.Columns("Bronze")), 0, grdOS.Columns("Bronze")))
    lngTotalNAT = CLng(IIf(Not IsNumeric(grdOS.Columns("Natural")), 0, grdOS.Columns("Natural")))
    '
    If lngTotal <> (lngTotalBRA + lngTotalBRI + lngTotalBRO + lngTotalNAT) Then
      strMsg = strMsg & "O somatório dos perfis devem ser igual a quantidade na linha " & intLinha + 1 & vbCrLf
    End If
  End If
  
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserLeituraFechaInc.ValidaCamposAnodOrigemLinha]"
...
Private Function ValidaCamposAnodOrigemAll() As Boolean
  ...
  If blnEncontrouErroLinha = False And blnEncontrouErro = False And blnCadastrou1Linha = False Then
    TratarErroPrevisto "Entre com ao menos 1 anodização para cadastro", "[frmUserLeituraFechaInc.ValidaCamposAnodOrigemAll]"
  End If
  grdOS.ReBind
  grdOS.SetFocus
...
Private Function ValidaCamposAnodOrigem(intLinha As Integer, intColuna As Integer) As Boolean
  ...
  '
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmAnodizacaoInc.ValidaCamposAnodOrigem]"
...
Private Sub Form_KeyPress(KeyAscii As Integer)
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmAnodizacaoInc.Form_KeyPress]"
End Sub
