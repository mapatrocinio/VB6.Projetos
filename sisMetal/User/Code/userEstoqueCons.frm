...
'Assume 1 REQUISIÇÃO DE MATERIAL
'BAIXA DE:
'Assume 2 ESTOQUE
'Assume 3 ESTOQUEINTERMEDIARIO
'Assume 4 CARDÁPIO
...
Private Sub cmdConfirmar_Click()
  ...
  Case 1 'Chamada de Requisição de Materiais
    frmUserRequisicaoMaterialInc.mskCodigo.Text = grdGeral.Columns(0).Value
  Case 2, 3 'Chamada de Baixa de estoque intermediário de consumo interno
    frmUserBaixaEstoqueInc.mskCodigo.Text = grdGeral.Columns(0).Value
  Case 4 'Cardápio
    frmUserCardapioResumoInc.mskCodigo.Text = grdGeral.Columns(0).Value
  End Select
  cmdCancelar_Click
End Sub
...
Private Sub cmdFiltrar_Click()
  ...
  If Len(Trim(txtItem.Text)) <> 0 Then
    strWhere = "DESCRICAO LIKE '*" & txtItem.Text & "*'"
  End If
  '
  COLUNASMATRIZ = 3
  LINHASMATRIZ = 0
...
Public Sub MontaMatriz(Optional strWhere As String)
  ...
  '
  If QuemChamou = 4 Then 'chamada do cardápio
    If Len(Trim(strWhere)) = 0 Then
      strWhere = "CODIGO LIKE 'C*'"
    Else
      strWhere = strWhere & " AND CODIGO LIKE 'C*'"
    End If
  End If
  strSql = "Select ESTOQUE.CODIGO, ESTOQUE.DESCRICAO, ESTOQUE.QTDESTOQUE From Estoque " & _
