...
Begin VB.Form frmFamiliaProdutoLis 
   ...
   Caption         =   "Lista de famílias de produtos"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7350
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma família de produtos !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma família de produtos para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da família de produtos " & grdGeral.Columns("Família de produto").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  If Not objRs.EOF Then
    ...
    TratarErroPrevisto "Família de produtos não pode ser excluída por constar referências na tabela de PRODUTO", "frmUserFamiliaProdutoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
