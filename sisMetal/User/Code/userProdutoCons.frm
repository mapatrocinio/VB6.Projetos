...
Begin VB.Form frmProdutoCons 
   Caption         =   "Consulta de itens do produto"
   ClientHeight    =   6360
   ClientLeft      =   2595
   ClientTop       =   3120
   ClientWidth     =   7935
   Icon            =   "userProdutoCons.frx":0000
   LinkTopic       =   "Form1"
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      Height          =   4335
      Left            =   0
      OleObjectBlob   =   "userProdutoCons.frx":000C
      TabIndex        =   2
      Top             =   960
      Width           =   7935
   End
   Begin VB.Label Label1 
      Caption         =   "Informe o nome ou código do produto"
      Height          =   375
      Left            =   240
      TabIndex        =   5
      Top             =   120
   ...
   End
End
Attribute VB_Name = "frmProdutoCons"
...
'Assume 1 Saída de Material
'Assume 2 Pedido Venda
...
Private Sub cmdConfirmar_Click()
  ...
  Case 0 'Chamada de Entrada de Materiais
    strCodigoProduto = grdGeral.Columns(0).Value
    frmEntradaMaterialInc.txtCodigoProduto.Text = grdGeral.Columns(0).Value
  Case 1 'Chamada de Entrada de Materiais
    strCodigoProduto = grdGeral.Columns(0).Value
    frmSaidaMaterialInc.txtCodigoProduto.Text = grdGeral.Columns(0).Value
  Case 2 'Chamada de Pedido Venda
    strCodigoProduto = grdGeral.Columns(0).Value
    frmPedidoItemVendaInc.txtCodProdutoFim.Text = grdGeral.Columns(0).Value
    frmPedidoItemVendaInc.txtNomProdutoFim.Text = grdGeral.Columns(1).Value
  End Select
  cmdCancelar_Click
End Sub
...
Private Sub cmdFiltrar_Click()
  ...
  If Len(Trim(txtItem.Text)) <> 0 Then
    strWhere = "(NOME LIKE '*" & txtItem.Text & "*' " & _
        " OR CODIGO LIKE '*" & txtItem.Text & "*') "
  End If
  '
  COLUNASMATRIZ = 3
  LINHASMATRIZ = 0
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmProdutoCons.grdGeral_UnboundReadDataEx]"
End Sub
