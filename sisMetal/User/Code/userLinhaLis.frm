...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    ...
    Exit Sub
  End If
  frmLinhaInc.Status = tpStatus_Alterar
  frmLinhaInc.lngTIPOLINHAID = grdGeral.Columns("ID").Value
  frmLinhaInc.Show vbModal
  
  If frmLinhaInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  'LINHA
  strSql = "Select * from LINHA WHERE TIPO_LINHAID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Linha não pode ser excluida, pois já está associado a linha-perfil.", "frmLinhaLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  '
  If MsgBox("Confirma exclusão da Linha " & grdGeral.Columns("Nome da linha").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
...
Private Sub cmdInserir_Click()
  ...
  frmLinhaInc.Status = tpStatus_Incluir
  frmLinhaInc.lngTIPOLINHAID = 0
  frmLinhaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT TIPO_LINHA.PKID, TIPO_LINHA.NOME " & _
            "FROM TIPO_LINHA " & _
            " ORDER BY TIPO.NOME "
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmLinhaLis.grdGeral_UnboundReadDataEx]"
End Sub
