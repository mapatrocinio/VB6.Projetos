...
Begin VB.Form frmCartaoLis 
   ...
   End
End
Attribute VB_Name = "frmCartaoLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmCartaoInc.lngCARTAOID = grdGeral.Columns("ID").Value
  frmCartaoInc.Status = tpStatus_Alterar
  frmCartaoInc.intQuemChamou = 1 'Chamada da Alteração (Exclusão)
  frmCartaoInc.Show vbModal
  
  If frmCartaoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  '
  strSql = "Select * from CONTACORRENTEPED where CARTAOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Cartão não pode ser excluido, pois já está associado a recebimento de pedido.", "frmUserCartaoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
...
Private Sub cmdInserir_Click()
  ...
  frmCartaoInc.Status = tpStatus_Incluir
  frmCartaoInc.intQuemChamou = 0
  frmCartaoInc.lngCARTAOID = 0
  frmCartaoInc.Show vbModal
