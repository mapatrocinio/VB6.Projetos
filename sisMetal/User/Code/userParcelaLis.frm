...
Begin VB.Form frmParcelaLis 
   ...
   End
End
Attribute VB_Name = "frmParcelaLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmParcelaInc.Status = tpStatus_Alterar
  frmParcelaInc.intChamada = 0
  frmParcelaInc.lngParcelaId = grdGeral.Columns("PKID").Value
  frmParcelaInc.Show vbModal
  
  If frmParcelaInc.blnRetorno Then
    MontaMatriz
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT PARCELA.PKID, PARCELA.PARCELA, PARCELA.DTVENCIMENTO, PARCELA.VRPARCELA " & _
            "FROM PARCELA " & _
            " WHERE PARCELA.CONTACORRENTEPEDID = " & Formata_Dados(lngCCId, tpDados_Longo) & _
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  curVrCC = 0
  curVrTotParc = 0
  strSql = "SELECT min(CONTACORRENTEPED.VALOR) AS VRCC, SUM(PARCELA.VRPARCELA) AS VRTOTPARCELA " & _
    "FROM CONTACORRENTEPED INNER JOIN PARCELA ON CONTACORRENTEPED.PKID = PARCELA.CONTACORRENTEPEDID " & _
    "WHERE CONTACORRENTEPED.PKID = " & Formata_Dados(lngCCId, tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
