...
Begin VB.Form frmCorInc 
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      Tabs            =   1
      TabHeight       =   520
      TabCaption(0)   =   "&Dados da cor"
      TabPicture(0)   =   "userCorInc.frx":0000
...
Private Sub cmdOK_Click()
  ...
  Case 0 'Inclusão/Alteração da Despesa
    ...
    strSql = "Select * From COR WHERE (NOME = " & Formata_Dados(txtNome.Text, tpDados_Texto, tpNulo_Aceita) & _
      " OR SIGLA = " & Formata_Dados(txtSigla.Text, tpDados_Texto, tpNulo_Aceita) & ") " & _
      " AND PKID <> " & Formata_Dados(lngCORID, tpDados_Longo, tpNulo_NaoAceita)
    Set objRs = objGer.ExecutarSQL(strSql)
    If Not objRs.EOF Then
      objRs.Close
      Set objRs = Nothing
      Set objGer = Nothing
      TratarErroPrevisto "Nome ou Sigla da cor já cadastrada", "cmdOK_Click"
    ...
    If Status = tpStatus_Alterar Then
      ...
      '
      objCor.AlterarCor txtSigla.Text, _
                          txtNome.Text, _
                          lngCORID
      blnRetorno = True
      blnFechar = True
    ...
    ElseIf Status = tpStatus_Incluir Then
      ...
      '
      objCor.IncluirCor txtSigla.Text, _
                          txtNome.Text
      'Limpar campos
      blnRetorno = True
      blnFechar = True
...
Private Function ValidaCampos() As Boolean
  ...
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmCorInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmCorInc.Form_Activate]"
End Sub
...
On Error GoTo trata
  ...
  ElseIf Status = tpStatus_Alterar Or Status = tpStatus_Consultar Then
    ...
    Set objCor = New busSisMetal.clsCor
    Set objRs = objCor.SelecionarCor(lngCORID)
    '
    If Not objRs.EOF Then
