...
Begin VB.Form frmVaraLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Metragem"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8550
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userVaraLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6480
   End
End
Attribute VB_Name = "frmVaraLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma metragem!", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmVaraInc.Status = tpStatus_Alterar
  frmVaraInc.lngVARAID = grdGeral.Columns("ID").Value
  frmVaraInc.Show vbModal
  
  If frmVaraInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  '
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma metragem para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  Set objGer = New busSisMetal.clsGeral
  '
  strSql = "Select * from LINHA where VARAID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Metragem não pode ser excluida, pois já está associada a linhas.", "frmVaraLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  '
  If MsgBox("Confirma exclusão da metragem " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objVara = New busSisMetal.clsVara
  
  objVara.ExcluirVara CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmVaraInc.Status = tpStatus_Incluir
  frmVaraInc.lngVARAID = 0
  frmVaraInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT VARA.PKID, VARA.NOME, VARA.VALOR " & _
            "FROM VARA " & _
            " ORDER BY VARA.NOME "
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmVaraLis.grdGeral_UnboundReadDataEx]"
End Sub
