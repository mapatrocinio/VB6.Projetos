...
Begin VB.Form frmOSFinalLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de OS Final"
   ClientHeight    =   6360
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11610
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      Height          =   5970
      Left            =   0
      OleObjectBlob   =   "userOSFinalLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   9645
   End
End
Attribute VB_Name = "frmOSFinalLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma OS Final!", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmOSFinalInc.Status = tpStatus_Alterar
  frmOSFinalInc.strOSNumero = strOSNumero
  frmOSFinalInc.lngOSID = lngOSID
  frmOSFinalInc.lngOSFINALID = grdGeral.Columns("ID").Value
  frmOSFinalInc.Show vbModal
  
  If frmOSFinalInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmOSFinalInc.Status = tpStatus_Incluir
  frmOSFinalInc.lngOSFINALID = 0
  frmOSFinalInc.lngOSID = lngOSID
  frmOSFinalInc.strOSNumero = strOSNumero
  frmOSFinalInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT OS_FINAL.PKID, OS.NUMERO , OS_FINAL.COMPLEMENTO, OS_FINAL.DATA, COR.NOME " & _
        "FROM OS_FINAL " & _
        " INNER JOIN OS ON OS.PKID = OS_FINAL.OSID " & _
        " LEFT JOIN COR ON COR.PKID = OS_FINAL.CORID " & _
        " WHERE OS.PKID = " & Formata_Dados(lngOSID, tpDados_Longo) & _
        " ORDER BY OS_FINAL.PKID DESC;"
  '
