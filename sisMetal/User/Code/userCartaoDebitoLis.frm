...
Begin VB.Form frmCartaoDebitoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de cartões de debito"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userCartaoDebitoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmCartaoDebitoLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmCartaoDebitoInc.lngCARTAOID = grdGeral.Columns("ID").Value
  frmCartaoDebitoInc.Status = tpStatus_Alterar
  frmCartaoDebitoInc.intQuemChamou = 1 'Chamada da Alteração (Exclusão)
  frmCartaoDebitoInc.Show vbModal
  
  If frmCartaoDebitoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objCartaoDebito As busSisMetal.clsCartaoDebito
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um cartão de débito para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from CONTACORRENTEPED where CARTAODEBITOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Cartão de Débito não pode ser excluido, pois já está associado a recebimento de pedido.", "frmUserCartaoDebitoDebitoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do Cartão de Débito " & grdGeral.Columns("Cartão").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objCartaoDebito = New busSisMetal.clsCartaoDebito
  
  objCartaoDebito.ExcluirCartaoDebito CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmCartaoDebitoInc.Status = tpStatus_Incluir
  frmCartaoDebitoInc.intQuemChamou = 0
  frmCartaoDebitoInc.lngCARTAOID = 0
  frmCartaoDebitoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT CARTAODEBITO.PKID, CARTAODEBITO.NUMERO, CARTAODEBITO.PERCTAXAADMIN, CARTAODEBITO.NOME " & _
            "FROM CARTAODEBITO " & _
            "ORDER BY NOME"
  '
