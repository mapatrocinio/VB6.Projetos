...
Public Sub Captura_Config()
  ...
  Dim intTotConfig      As Integer
  '
  Set objGeral = New busSisMetal.clsGeral
  '
  intTotConfig = 0
  strSql = "Select count(*) as total from Configuracao "
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    If IsNumeric(objRs.Fields("total").Value) Then
      intTotConfig = objRs.Fields("total").Value
    End If
  End If
  objRs.Close
  Set objRs = Nothing
  
'''  blnClienteTrabMultiplasConfig = False
'''  If intTotConfig > 1 Then
...
'''
  ...
  '-----------------------
  strSql = "Select * from Configuracao where PKID IN (SELECT MIN(PKID) FROM CONFIGURACAO)"
  Set objRs = objGeral.ExecutarSQL(strSql)
  '
  giQtdDiasPedido = 1
  If objRs.EOF Then
    gsCaminhoImagemCompra = "'"
    giQtdDiasVenda = 1
  Else
    gsCaminhoImagemCompra = objRs.Fields("CAMINHOIMAGEMCOMPRA").Value & "" & IIf(Right(objRs.Fields("CAMINHOIMAGEMCOMPRA").Value & "", 1) <> "\", "\", "")
    giQtdDiasVenda = objRs.Fields("QTDDIASVENDA").Value
  End If
  objRs.Close
  Set objRs = Nothing
  '
...
Public Sub TratarErro(ByVal pNumero As Long, _
  ...
  Dim intI       As Integer
  ' NOVO - VERIFICAR SE É UM ERRO A SER TRATADO
  If InStr(1, pDescricao, "SISMETAL980") <> 0 Then
    'Erro tratado
    TratarErroPrevisto Replace(Left(pDescricao, InStr(1, pDescricao, "|") - 2), "SISMETAL980", ""), pSource
    Exit Sub
  End If
  intI = FreeFile
  Open App.Path & "\SisMetal.txt" For Append As #intI
  ...
  MsgBox "O Seginte Erro Ocorreu: " & vbCrLf & vbCrLf & _
    ...
    "Erro gravado no arquivo: " & App.Path & "\SisMetal.txt" & vbCrLf & vbCrLf & _
...
Public Sub Monta_Menu(pAcao As Integer)
  ...
  '
  'Desabilita Menu
  frmMDI.snuCadastro(0).Visible = False
  frmMDI.snuCompra(0).Visible = False
  frmMDI.snuVenda(0).Visible = False
  frmMDI.snuEstoquePrinc(0).Visible = False
  ...
  If pAcao = 1 Then 'Monta Conexão
    ...
    Case gsAdmin, gsDiretor, gsGerente, gsCompra, gsFinanceiro, gsVendedor
      'Acesso Completo
      frmMDI.snuCadastro(0).Visible = True
      frmMDI.snuCompra(0).Visible = True
      frmMDI.snuVenda(0).Visible = True
      frmMDI.snuEstoquePrinc(0).Visible = True
      frmMDI.snuFinanceiro(0).Visible = True
      frmMDI.snuRelatorio(0).Visible = True
      '
      'frmMDI.mnuArquivo(2).Visible = True 'usuário
      frmMDI.mnuArquivo(2).Visible = True 'papel de parede
      frmMDI.mnuArquivo(3).Visible = True 'configuração
      frmMDI.mnuArquivo(4).Visible = True 'separador
    Case gsLoja, gsCaixa
      'Acesso Vendedor de loja
      'frmMDI.snuCadastro(0).Visible = True
      'frmMDI.snuCompra(0).Visible = True
      frmMDI.snuVenda(0).Visible = True
      'frmMDI.snuEstoquePrinc(0).Visible = True
      ...
      'frmMDI.mnuArquivo(2).Visible = True 'usuário
      frmMDI.mnuArquivo(2).Visible = True 'papel de parede
      'frmMDI.mnuArquivo(5).Visible = True 'configuração
      frmMDI.mnuArquivo(4).Visible = True 'separador
    Case gsCaixa
    End Select
  End If
...
Public Function TestaCPF(CPF As String, _
  ...
  strMsg = ""
  'VALIDAÇÕES BÁSICAS DO CPF
  If Len(CPF) <> 11 Then
    TestaCPF = False
    strMsg = "Informar o CPF com 11 posições e apenas números"
    Exit Function
  End If
