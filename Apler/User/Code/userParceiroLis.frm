...
Private Sub cmdAlterar_Click()
  ...
  frmUserParceiroInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserParceiroInc.Show vbModal
  
  If frmUserParceiroInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um parceiro para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do parceiro " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT PARCEIRO.PKID, PARCEIRO.NOME, CONVENIO.NOME "
  strSql = strSql & " FROM PARCEIRO LEFT JOIN CONVENIO ON CONVENIO.PKID = PARCEIRO.CONVENIOID " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserParceiro.grdGeral_UnboundReadDataEx]"
End Sub
