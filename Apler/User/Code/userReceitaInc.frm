...
Private Sub cmdOK_Click()
  ...
  Case 0 'Inclusão/Alteração da Receita
    ...
    If cboLivro.Text <> "" Then
      ...
      Set objRs = Nothing
    End If
    strSql = "Select EMPRESACTRTO.PKID From EMPRESACTRTO INNER JOIN EMPRESACTA ON EMPRESACTA.PKID = EMPRESACTRTO.EMPRESACTAID " & _
            " WHERE EMPRESACTA.PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
...
On Error GoTo trata
  ...
  strSql = "SELECT EMPRESACTRTO.DESCRICAO FROM EMPRESACTRTO " & _
                " INNER JOIN EMPRESACTA ON EMPRESACTA.PKID = EMPRESACTRTO.EMPRESACTAID " & _
                " WHERE EMPRESACTA.PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
