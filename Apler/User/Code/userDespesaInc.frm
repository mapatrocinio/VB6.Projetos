   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.Frame Frame1 
            ...
            Top             =   600
            Width           =   7335
            Begin VB.ComboBox cboConfiguracao 
               Height          =   315
               Left            =   1560
...
Public blnTrabalhaComFuncAssoc        As Boolean

Private Sub cboConfiguracao_LostFocus()
  Pintar_Controle cboConfiguracao, tpCorContr_Normal
End Sub
...
Private Sub cmdOk_Click()
  On Error GoTo trata
  Dim objForm                 As Apler.frmUserLocContaCorrente
  Dim strSql                  As String
  Dim strMsgErro              As String
  Dim objRs                   As ADODB.Recordset
  Dim objDespesa              As busApler.clsDespesa
  ...
  Case 0 'Inclusão/Alteração da Despesa
    ...
    'Obter funcionário
    lngFUNCIONARIOID = 0
    If chkVale.Value = 1 And blnTrabalhaComFuncAssoc = True Then
      strSql = "Select PKID From FUNCIONARIO WHERE NOME = " & Formata_Dados(cboFuncionario.Text, tpDados_Texto)
    ...
    If mskGrupo.ClipText <> "" Or mskSubGrupo.ClipText <> "" Then
      ...
      Set objRs = Nothing
    End If
    'tudo ok, pegar configuração
    strSql = "SELECT CONFIGURACAO.PKID FROM CONFIGURACAO"
    strSql = strSql & " WHERE CONFIGURACAO.TITULO = " & Formata_Dados(cboConfiguracao.Text, tpDados_Texto)
    Set objRs = clsGer.ExecutarSQL(strSql)
    If Not objRs.EOF Then
...
'''    '--------------------------------
    ...
    Set objDespesa = New busApler.clsDespesa
    ...
    If Status = tpStatus_Alterar Then
      ...
      objForm.lngLOCDESPVDAEXTID = lngDESPESAID
      objForm.intGrupo = 0
      objForm.strNumeroAptoPrinc = ""
      objForm.Status = tpStatus_Incluir
      objForm.strStatusLanc = "DE"
    ...
    ElseIf Status = tpStatus_Incluir Then
      ...
      If chkPgto.Value = False Then
        ...
        lngCCId = objCC.InserirCC(lngDESPESAID, _
                          RetornaCodTurnoCorrente, _
                          Format(Now, "DD/MM/YYYY hh:mm"), _
                          mskValor.Text, _
                          "C", _
...
Private Function ValidaCampos() As Boolean
  ...
  If chkVale.Value = False Then
    ...
    End If
  Else
    If blnTrabalhaComFuncAssoc = True Then
  ...
  If Len(cboConfiguracao.Text) = 0 Then
    strMsg = strMsg & "Selecionar uma configuração" & vbCrLf
...
Private Sub cmdPagamento_Click()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserDespesaInc.cmdPagamento_Click]"
End Sub
...
On Error GoTo trata
  ...
  LerFigurasAvulsas cmdPagamento, "InfFinanc.ico", "InfFinancDown.ico", "Extrato de Pagamento"
  '
  strSql = "SELECT FORMAPGTO FROM FORMAPGTO ORDER BY FORMAPGTO;"
  PreencheCombo cboFormaPgto, strSql, False, True
  strSql = "SELECT NOME FROM FUNCIONARIO ORDER BY NOME;"
  PreencheCombo cboFuncionario, strSql, False, True
  If cboFuncionario.ListCount > 1 Then
    'Há funcionários cadastrados
    blnTrabalhaComFuncAssoc = True
  Else
    blnTrabalhaComFuncAssoc = False
  End If
  If blnTrabalhaComFuncAssoc = False Then
    'Desabilita campos
    Label1(1).Visible = False
    cboFuncionario.Visible = False
  End If
  'Configuracao
  strSql = "Select TITULO from CONFIGURACAO ORDER BY TITULO"
  PreencheCombo cboConfiguracao, strSql, False, True
  If blnClienteTrabMultiplasConfig = False Then
    cboConfiguracao.Visible = False
  ...
  If Status = tpStatus_Incluir Then
    ...
    '
    INCLUIR_VALOR_NO_COMBO RetornaTituloConfiguracao, cboConfiguracao
    If blnTrabalhaComFuncAssoc = True Then
      'Tratar campo
      Label1(1).Enabled = False
      cboFuncionario.Enabled = False
  ...
  ElseIf Status = tpStatus_Alterar Then
    ...
    If Not objRs.EOF Then
      ...
      INCLUIR_VALOR_NO_COMBO objRs.Fields("TITULO").Value, cboConfiguracao
      If blnTrabalhaComFuncAssoc = True Then
        'Tratar campo
        chkVale_Click
