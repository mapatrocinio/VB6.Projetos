...
Private Sub cmdOK_Click()
  ...
  Case 0 'Inclusão/Alteração da Despesa
    ...
    strSql = "Select PKID From CONTA " & _
          " WHERE DESCRICAO = " & Formata_Dados(cboDe.Text, tpDados_Texto, tpNulo_NaoAceita) & _
    ...
    If strStatus = "M" Then
      'Movimentacao
      strSql = "Select PKID From CONTA " & _
              " WHERE DESCRICAO = " & Formata_Dados(cboPara.Text, tpDados_Texto, tpNulo_NaoAceita) & _
              " AND PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo)
      Set objRs = clsGer.ExecutarSQL(strSql)
      If objRs.EOF Then
        objRs.Close
    ...
    ElseIf Status = tpStatus_Incluir Then
      ...
      If Len(Trim(txtDocumento.Text)) = 0 And strStatus = "M" Then
        strDocumento = RetornaGravaSequencialV1("SEQUENCIALMOV" & glParceiroId)
      Else
...
On Error GoTo trata
  ...
  '
  strSql = "SELECT DESCRICAO FROM CONTA " & _
