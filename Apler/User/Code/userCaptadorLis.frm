...
Begin VB.Form frmUserCaptadorLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userCaptadorLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8160
   End
End
Attribute VB_Name = "frmUserCaptadorLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um captador !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserCaptadorInc.Status = tpStatus_Alterar
  frmUserCaptadorInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserCaptadorInc.Show vbModal
  
  If frmUserCaptadorInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  '
  On Error GoTo trata
  'Cadastro de Captadors
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um captador para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do captador " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objCaptador = New busApler.clsCaptador
  objCaptador.ExcluirCaptador CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserCaptadorInc.Status = tpStatus_Incluir
  frmUserCaptadorInc.lngPKID = 0
  frmUserCaptadorInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de Captadors
  strSql = "SELECT CAPTADOR.PKID, CAPTADOR.NOME, CASE CAPTADOR.STATUS WHEN 'A' THEN 'Ativo' ELSE 'Inativo' END "
  strSql = strSql & " FROM CAPTADOR " & _
            " ORDER BY CAPTADOR.NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserCaptador.grdGeral_UnboundReadDataEx]"
End Sub
