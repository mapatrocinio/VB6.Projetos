...
Begin VB.Form frmUserSaldoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Saldos"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8550
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userSaldoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6480
   End
End
Attribute VB_Name = "frmUserSaldoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um saldo !", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  frmUserSaldoInc.Status = tpStatus_Alterar
  frmUserSaldoInc.lngSALDOID = grdGeral.Columns("ID").Value
  frmUserSaldoInc.Show vbModal
  
  If frmUserSaldoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objSaldo      As busApler.clsSaldo
  '
  On Error GoTo trata
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    MsgBox "Selecione um Saldo para exclusão.", vbExclamation, TITULOSISTEMA
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão do Saldo " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  'OK
  Set objSaldo = New busApler.clsSaldo
  
  objSaldo.ExcluirSaldo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserSaldoInc.Status = tpStatus_Incluir
  frmUserSaldoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT SALDO.PKID, SALDO.DESCRICAO, SALDO.PERCENTUAL " & _
            "FROM SALDO " & _
            " WHERE PARCEIROID = " & Formata_Dados(glParceiroId, tpDados_Longo) & _
            " ORDER BY SALDO.DESCRICAO "
  '
