...
Begin VB.Form frmUserPredioLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de prédio"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userPredioLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserPredioLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um Prédio !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserPredioInc.lngPredioID = grdGeral.Columns("ID").Value
  frmUserPredioInc.Status = tpStatus_Alterar
  frmUserPredioInc.Show vbModal
  
  If frmUserPredioInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione um Prédio para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from SALA where PREDIOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Prédio não pode ser excluido, pois já está associado a sala.", "frmUserPredioLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do Prédio " & grdGeral.Columns("Prédio").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objPredio.ExcluirPredio CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserPredioInc.Status = tpStatus_Incluir
  frmUserPredioInc.lngPredioID = 0
  frmUserPredioInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT PREDIO.PKID, PREDIO.NOME " & _
            "FROM PREDIO " & _
            "ORDER BY NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserPredioLis.grdGeral_UnboundReadDataEx]"
End Sub
