...
Begin VB.Form frmUserProcedimentoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Procedimento"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10500
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userProcedimentoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8550
   End
End
Attribute VB_Name = "frmUserProcedimentoLis"
...
Private Sub cmdAlterar_Click()
  ...
  frmUserProcedimentoInc.lngPROCEDIMENTOID = grdGeral.Columns("ID").Value
  frmUserProcedimentoInc.Status = tpStatus_Alterar
  frmUserProcedimentoInc.Show vbModal
  
  If frmUserProcedimentoInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  '
  strSql = "Select * from PRESTADORPROCEDIMENTO where PROCEDIMENTOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Procedimento não pode ser excluido, pois já está associado a prestador de serviço.", "frmUserGarcomLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do Procedimento " & grdGeral.Columns("Procedimento").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objProcedimento.ExcluirProcedimento CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserProcedimentoInc.Status = tpStatus_Incluir
  frmUserProcedimentoInc.lngPROCEDIMENTOID = 0
  frmUserProcedimentoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT PROCEDIMENTO.PKID, PROCEDIMENTO.PROCEDIMENTO, PROCEDIMENTO.INDCONSULTA, PROCEDIMENTO.INDACEITAVALOR, TIPOPROCEDIMENTO.TIPOPROCEDIMENTO, PROCEDIMENTO.VALOR " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserProcedimentoLis.grdGeral_UnboundReadDataEx]"
End Sub
