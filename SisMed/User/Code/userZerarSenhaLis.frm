...
Begin VB.Form frmUserZerarSenhaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de prestadores"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userZerarSenhaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserZerarSenhaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um prestador para zerar a senha !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  Set objGeral = Nothing
  If MsgBox("Confirma zerar a senha do prestador " & grdGeral.Columns("Prestador").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objGeral = New busSisMed.clsGeral
  strSql = "DELETE FROM SEQUENCIALPRESTADOR " & _
          " WHERE PRESTADORID = " & Formata_Dados(grdGeral.Columns("ID").Value, tpDados_Longo) & _
          " AND DATA = " & Formata_Dados(grdGeral.Columns("Data").Value, tpDados_DataHora)
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT SEQUENCIALPRESTADOR.PRESTADORID, SEQUENCIALPRESTADOR.DATA, PRONTUARIO.NOME, SEQUENCIALPRESTADOR.SEQUENCIAL " & _
          " FROM SEQUENCIALPRESTADOR " & _
          " INNER JOIN PRONTUARIO ON PRONTUARIO.PKID = SEQUENCIALPRESTADOR.PRESTADORID " & _
          " WHERE SEQUENCIALPRESTADOR.DATA >= " & Formata_Dados(strDataIni, tpDados_DataHora) & _
          " AND SEQUENCIALPRESTADOR.DATA < " & Formata_Dados(strDataFim, tpDados_DataHora) & _
          " ORDER BY SEQUENCIALPRESTADOR.DATA, PRONTUARIO.NOME"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserZerarSenhaLis.grdGeral_UnboundReadDataEx]"
End Sub
