...
Begin VB.Form frmUserEspecialidadeLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Especialidade"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userEspecialidadeLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserEspecialidadeLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma Especialidade !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserEspecialidadeInc.lngESPECIALIDADEID = grdGeral.Columns("ID").Value
  frmUserEspecialidadeInc.Status = tpStatus_Alterar
  frmUserEspecialidadeInc.Show vbModal
  
  If frmUserEspecialidadeInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma Especialidade para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from PRESTADORESPECIALIDADE where ESPECIALIDADEID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Especialidade não pode ser excluida, pois já está associada a prestador de serviço.", "frmUserGarcomLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão da Especialidade " & grdGeral.Columns("Especialidade").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objEspecialidade.ExcluirEspecialidade CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserEspecialidadeInc.Status = tpStatus_Incluir
  frmUserEspecialidadeInc.lngESPECIALIDADEID = 0
  frmUserEspecialidadeInc.Show vbModal
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserEspecialidadeLis.grdGeral_UnboundReadDataEx]"
End Sub
