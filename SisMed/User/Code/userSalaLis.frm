...
Begin VB.Form frmUserSalaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de sala"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10080
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userSalaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8160
   End
End
Attribute VB_Name = "frmUserSalaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma sala!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserSalaInc.Status = tpStatus_Alterar
  frmUserSalaInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserSalaInc.Show vbModal
  
  If frmUserSalaInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  'Cadastro de salas
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma sala para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  If MsgBox("Confirma exclusão da sala " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objSala = New busSisMed.clsSala
  objSala.ExcluirSala CLng(grdGeral.Columns("PKID").Value)
  Set objSala = Nothing
  '
  MontaMatriz
  grdGeral.Bookmark = Null
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserSalaInc.Status = tpStatus_Incluir
  frmUserSalaInc.lngPKID = 0
  frmUserSalaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de sala
  strSql = "SELECT SALA.PKID, SALA.NUMERO, PREDIO.NOME, SALA.ANDAR, SALA.TELEFONE "
  strSql = strSql & " FROM SALA INNER JOIN PREDIO ON PREDIO.PKID = SALA.PREDIOID " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserSala.grdGeral_UnboundReadDataEx]"
End Sub
