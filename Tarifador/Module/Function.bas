...
Option Explicit
Public Const gsCaminhoMesaTel = "C:\Sismotel\LIGACOES\TRFTXT1.TXT"

'Dll de Proteção do sistema
'Global clsProtec    As Protec.clsProtec
Enum TpTipoBD
  TpTipoBD_ACCESS = 0
  TpTipoBD_SQL = 1
End Enum
  ...
  Case TpTipoDados.tpDados_DataHora
    ...
    ElseIf intBancoDados = TpTipoBD.TpTipoBD_SQL Then
      ...
      If Len(pValor & "") = 10 Then
        If Mid(pValor & "", 1, 2) <> "__" Then
          'Data no Formato DD/MM/YYYY
          sData = "'" & Mid(pValor, 7, 4) & "/" & Mid(pValor, 4, 2) & "/" & Mid(pValor, 1, 2) & "'"
        Else
          sData = "null"
        End If
      ElseIf Len(pValor & "") = 16 Then
        ...
        If Mid(pValor & "", 1, 2) <> "__" Then
          'Data no Formato DD/MM/YYYY
          sData = "'" & Mid(pValor, 7, 4) & "/" & Mid(pValor, 4, 2) & "/" & Mid(pValor, 1, 2) & " " & Mid(pValor, 12, 2) & ":" & Mid(pValor, 15, 2) & "'"
        Else
          sData = "null"
        End If
      Else
...
trata:
  Err.Raise Err.Number, _
            "[mdlUserFunction.Formata_Dados]", _
            Err.Description
End Function
