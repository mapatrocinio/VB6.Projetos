...
Begin VB.Form frmUserBoletoAtendInc 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cadastro de boleto para atendente"
   ClientHeight    =   4410
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10470
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      TabPicture(0)   =   "userBoletoAtendInc.frx":0000
   ...
   End
End
Attribute VB_Name = "frmUserBoletoAtendInc"
...
trata:
  Err.Raise Err.Number, _
            "[frmUserBoletoAtendInc.LimparCampos]", _
            Err.Description
End Sub
...
Private Sub cmdOk_Click()
  ...
  Set objRs = Nothing
  '
  'Verifica se há turno aberto para o atendente
  lngCAIXAATENDID = 0
  strSql = "SELECT CAIXAATEND.PKID FROM CAIXAATEND " & _
      " WHERE CAIXAATEND.PKID = " & Formata_Dados(RetornaCodTurnoCorrenteAtend(lngATENDENTEID), tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    lngCAIXAATENDID = objRs.Fields("PKID").Value
  Else
    'Não há caiatend aberto
    strMsg = "Não há turno aberto para este atendente."
...
Private Function ValidaCampos() As Boolean
  ...
  End If
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserBoletoAtendInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
trata:
  Err.Raise Err.Number, _
            Err.Source & ".[frmUserBoletoAtendInc.ValidaCampos]", _
            Err.Description
End Function
...
Private Sub Form_Activate()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBoletoAtendInc.Form_Activate]"
End Sub
...
Private Sub Form_Load()
  ...
  ElseIf Status = tpStatus_Alterar Or Status = tpStatus_Consultar Then
    ...
    '
'''    Set objBoletoAtend = New busSisMaq.clsBoletoAtend
'''    Set objRs = objBoletoAtend.SelecionarBoletoAtendPeloPkid(lngPKID)
'''    '
'''    If Not objRs.EOF Then
