...
Private Sub cmdOk_Click()
  ...
  lngCAIXAARRECID = 0
  strSql = "SELECT CAIXAARREC.PKID FROM CAIXAARREC WHERE " & _
      " CAIXAARREC.ARRECADADORID = " & Formata_Dados(lngARRECADADORID, tpDados_Longo) & _
      " AND CAIXAARREC.TURNOENTRADAID = " & Formata_Dados(RetornaCodTurnoCorrente, tpDados_Longo)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    lngCAIXAARRECID = objRs.Fields("PKID").Value
'''  Else
'''    'Não há caixa atend aberto
'''    strMsg = "Não há turno aberto para este arrecadador."
