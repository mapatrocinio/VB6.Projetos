...
Begin VB.Form frmUserEmpTrocaLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de empréstimo/troca"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10590
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userEmpTrocaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   8625
   End
End
Attribute VB_Name = "frmUserEmpTrocaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um empréstimo/troca!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserEmpTrocaInc.Status = tpStatus_Alterar
  frmUserEmpTrocaInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserEmpTrocaInc.Show vbModal
  
  If frmUserEmpTrocaInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  'Cadastro de Séries
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione um empréstimo/troca para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  'OK
  If MsgBox("Confirma exclusão do empréstimo/troca de " & grdGeral.Columns("Nome").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objEmpTroca = New busSisMaq.clsEmpTroca
  objEmpTroca.ExcluirEmpTroca CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserEmpTrocaInc.Status = tpStatus_Incluir
  frmUserEmpTrocaInc.lngPKID = 0
  frmUserEmpTrocaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de EmpTroca
  strSql = "SELECT EMPTROCA.PKID, EMPTROCA.NOME, TIPOPGTO.TIPOPGTO, EMPTROCA.DATA, EMPTROCA.VALOR "
  strSql = strSql & " FROM EMPTROCA " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserEmpTroca.grdGeral_UnboundReadDataEx]"
End Sub
