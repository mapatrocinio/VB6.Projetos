...
Begin VB.Form frmUserEntradaAtendLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de entrada"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8580
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userEntradaAtendLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6585
   End
End
Attribute VB_Name = "frmUserEntradaAtendLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma entrada para o atendente !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserEntradaAtendInc.Status = tpStatus_Alterar
  frmUserEntradaAtendInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserEntradaAtendInc.Show vbModal
  
  If frmUserEntradaAtendInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  'Entrada Atendente
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma entrada para atendente para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  'OK
  If MsgBox("Confirma exclusão da entrada de " & grdGeral.Columns("Data").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objEntradaAtend = New busSisMaq.clsEntradaAtend
  objEntradaAtend.ExcluirEntradaAtend CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserEntradaAtendInc.Status = tpStatus_Incluir
  frmUserEntradaAtendInc.lngPKID = 0
  frmUserEntradaAtendInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de EntradaAtend
  strSql = "SELECT ENTRADAATEND.PKID, PESSOA.NOME, ENTRADAATEND.DATA, ENTRADAATEND.VALOR "
  strSql = strSql & " FROM ENTRADAATEND " & _
            ...
            " WHERE ENTRADAATEND.TURNOENTRADAID = " & Formata_Dados(RetornaCodTurnoCorrente, tpDados_Longo) & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserEntradaAtend.grdGeral_UnboundReadDataEx]"
End Sub
