...
Private Sub cmdAlterar_Click()
  On Error GoTo trata
  'Dim objFormProcPrestador As SisMaq.frmUserProcPrestadorInc
...
'''    End If
'''
'''    Set objFormProcPrestador = New SisMaq.frmUserProcPrestadorInc
...
Private Sub cmdExcluir_Click()
  On Error GoTo trata
'''  Dim objPrestProcedimento      As busSisMaq.clsPrestProcedimento
'''  '
'''  Select Case tabDetalhes.Tab
...
'''      Exit Sub
'''    End If
'''    Set objPrestProcedimento = New busSisMaq.clsPrestProcedimento
...
Private Sub cmdOK_Click()
  Dim objPessoa                  As busSisMaq.clsPessoa
  Dim objFuncionario             As busSisMaq.clsFuncionario
'''  Dim objPaciente                As busSisMaq.clsPaciente
'''  Dim objPrestador               As busSisMaq.clsPrestador
  ...
  Set objPessoa = New busSisMaq.clsPessoa
...
'''    'Paciente
'''    Set objPaciente = New busSisMaq.clsPaciente
...
'''    End If
  If IcPessoa = tpIcPessoa_Func Then
    'Funcionario
    Set objFuncionario = New busSisMaq.clsFuncionario
    ...
    Else
      ...
      objFuncionario.InserirFuncionario lngPKID, _
                                      ...
                                      Encripta(UCase$(txtNovaSenha.Text)), _
                                      strFuncExcluido
      'Após inserção do funcionário, verificar o nível apra inclusão
      
      Select Case cboNivel.Text
      Case "ADMINISTRADOR", "DIRETOR"
        objFuncionario.InserirGerente (lngPKID)
        objFuncionario.InserirAtendente (lngPKID)
        objFuncionario.InserirArrecadador (lngPKID)
        objFuncionario.InserirDono (lngPKID)
        objFuncionario.InserirLeiturista (lngPKID)
        objFuncionario.InserirCaixa (lngPKID)
      Case "GERENTE"
        objFuncionario.InserirGerente (lngPKID)
        objFuncionario.InserirAtendente (lngPKID)
        objFuncionario.InserirArrecadador (lngPKID)
...
'''    'Prestador
'''    Set objPrestador = New busSisMaq.clsPrestador
...
'''  End If
  ...
  If IcPessoa = tpIcPessoa.tpIcPessoa_Func Then
    If Not Valida_String(cboNivel, TpObrigatorio, blnSetarFocoControle) Then
      ...
      tabDetalhes.Tab = 0
    End If
    If Left(cboNivel.Text, 3) <> "SEM" And Left(cboNivel.Text, 3) <> "DON" Then
...
'''  Dim objPrestador                As busSisMaq.clsPrestador
  ...
  cboNivel.AddItem "ARRECADADOR"
  cboNivel.AddItem "ATENDENTE"
  cboNivel.AddItem "CAIXA"
  If gsNivel = gsAdmin Or gsNivel = gsDiretor Then _
    cboNivel.AddItem "DIRETOR"
  cboNivel.AddItem "DONO"
  cboNivel.AddItem "FINANCEIRO"
  If gsNivel = gsAdmin Or gsNivel = gsDiretor Then _
    cboNivel.AddItem "GERENTE"
  If gsNivel = gsAdmin Or gsNivel = gsDiretor Then _
    cboNivel.AddItem "LEITURISTA"
...
Public Sub ESPEC_MontaMatriz()
  ...
  '
  strSql = "SELECT PRESTADORTIPO.PKID, TIPO.TIPO " & _
          "FROM PRESTADORTIPO INNER JOIN TIPO ON TIPO.PKID = PRESTADORTIPO.TIPOID " & _
          "WHERE PRESTADORTIPO.PESSOAID = " & Formata_Dados(lngPKID, tpDados_Longo) & _
          " ORDER BY TIPO.TIPO"
