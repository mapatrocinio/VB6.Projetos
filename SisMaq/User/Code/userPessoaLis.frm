...
Private Sub cmdAlterar_Click()
  ...
  If grdGeral.Columns("Nível").Value & "" = "ADMINISTRADOR" And gsNivel <> gsAdmin Then
    MsgBox "Apenas o administrador tem acesso a esta conta!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  If grdGeral.Columns("Nível").Value & "" = "DIRETOR" And gsNivel <> gsAdmin And gsNivel <> gsDiretor Then
    MsgBox "Apenas o administrador e o diretor tem acesso a esta conta!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  If grdGeral.Columns("Nível").Value & "" = "GERENTE" And gsNivel <> gsAdmin And gsNivel <> gsDiretor Then
    MsgBox "Apenas o administrador e o diretor tem acesso a esta conta!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  If grdGeral.Columns("Nível").Value & "" = "LEITURISTA" And gsNivel <> gsAdmin And gsNivel <> gsDiretor Then
    MsgBox "Apenas o administrador e o diretor tem acesso a esta conta!", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
...
Public Sub MontaMatriz()
  ...
  strSql = "SELECT PESSOA.PKID, PESSOA.NOME, " & _
    " case FUNCIONARIO.NIVEL when 'SEM' then 'SEM ACESSO AO SISTEMA' when 'FIN' then 'FINANCEIRO' when 'ADM' then 'ADMINISTRADOR' when 'DIR' then 'DIRETOR' when 'GER' then 'GERENTE' when 'CAI' then 'CAIXA' when 'ARR' then 'ARRECADADOR' when 'ATE' then 'ATENDENTE' when 'DON' then 'DONO' when 'LEI' then 'LEITURISTA' end AS DESCNIVEL, " & _
    "PESSOA.DTNASCIMENTO FROM PESSOA "
  If IcPessoa = tpIcPessoa_Func Then
