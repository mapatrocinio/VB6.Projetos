...
Begin VB.Form frmUserRelMedAtendente 
   Caption         =   "Relatório de Medições de Atendente"
   ClientHeight    =   4485
   ClientLeft      =   450
   ClientTop       =   1920
   ClientWidth     =   6105
   ...
   End
End
Attribute VB_Name = "frmUserRelMedAtendente"
...
Private Sub cmdRelatorio_Click()
  ...
  If cboAtendente.Text <> "" Then
    strSql = "SELECT FUNCIONARIO.PESSOAID PKID FROM PESSOA " & _
        ...
        " AND FUNCIONARIO.USUARIO = " & Formata_Dados(cboAtendente.Text, tpDados_Texto)
    Set objRs = objGeral.ExecutarSQL(strSql)
    If Not objRs.EOF Then
  ...
  If mskBoleto.Text <> "" Then
    strSql = "SELECT BOLETOATEND.PKID FROM BOLETOATEND " & _
        " WHERE BOLETOATEND.STATUS <> " & Formata_Dados("C", tpDados_Texto) & _
        " AND BOLETOATEND.NUMERO = " & Formata_Dados(mskBoleto.Text, tpDados_Longo)
    Set objRs = objGeral.ExecutarSQL(strSql)
    If Not objRs.EOF Then
  ...
  End If
  If lngATENDENTEID = 0 Then
    Report1.Formulas(3) = "ATENDENTEID = True = true"
  Else
    Report1.Formulas(3) = "ATENDENTEID = {FUNCIONARIO.PESSOAID} = " & lngATENDENTEID
  End If
  If lngBOLETOID = 0 Then
    Report1.Formulas(4) = "BOLETOID = True = true"
  Else
    Report1.Formulas(4) = "BOLETOID = {BOLETOATEND.PKID} = " & lngBOLETOID
  End If
  '
  '
  Report1.Action = 1
...
Private Sub Form_Load()
  ...
  Report1.ReportFileName = gsReportPath & "MedicaoAtendente.rpt"
  '
