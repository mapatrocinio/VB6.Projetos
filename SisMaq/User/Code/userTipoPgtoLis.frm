...
Begin VB.Form frmUserTipoPgtoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de tipod de pagamento"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6450
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userTipoPgtoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   4440
   End
End
Attribute VB_Name = "frmUserTipoPgtoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    ...
    Exit Sub
  End If
  frmUserTipoPgtoInc.Status = tpStatus_Alterar
  frmUserTipoPgtoInc.lngTIPOPGTOID = grdGeral.Columns("ID").Value
  frmUserTipoPgtoInc.Show vbModal
  
  If frmUserTipoPgtoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  End If
  '
  If MsgBox("Confirma exclusão do tipo de pagamento " & grdGeral.Columns("Descrição").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then Exit Sub
  Set objGeral = New busSisMaq.clsGeral
  strSql = "SELECT * FROM EMPTROCA WHERE TIPOPGTOID = " & Formata_Dados(CLng(grdGeral.Columns("ID").Value), tpDados_Longo, tpNulo_Aceita)
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGeral = Nothing
    TratarErroPrevisto "Não é possível excluir este tipo de pagamento, pois existem empréstimos/trocas cadastrados para ele.", "[cmdExcluir_Click]"
    Exit Sub
  End If
  objRs.Close
  Set objRs = Nothing
  ...
  objTipoPgto.ExcluirTipoPgto CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserTipoPgtoInc.Status = tpStatus_Incluir
  frmUserTipoPgtoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT TIPOPGTO.PKID, TIPOPGTO.TIPOPGTO " & _
            "FROM TIPOPGTO " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserTipoPgto.grdGeral_UnboundReadDataEx]"
End Sub
