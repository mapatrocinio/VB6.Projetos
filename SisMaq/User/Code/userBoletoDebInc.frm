...
Begin VB.Form frmUserBoletoDebInc 
   ...
   Begin TabDlg.SSTab tabDetalhes 
      ...
      TabPicture(0)   =   "userBoletoDebInc.frx":0000
      ...
      Begin VB.Frame Frame1 
         ...
         Begin TrueDBGrid60.TDBGrid grdBL 
            Height          =   3555
            Left            =   90
            OleObjectBlob   =   "userBoletoDebInc.frx":001C
            TabIndex        =   3
            Top             =   240
            Width           =   5415
         End
         Begin TrueDBGrid60.TDBGrid grdBLOrigem 
            Height          =   3555
            Left            =   5850
            OleObjectBlob   =   "userBoletoDebInc.frx":735B
            TabIndex        =   8
            TabStop         =   0   'False
            Top             =   240
            Width           =   6435
   ...
   End
End
Attribute VB_Name = "frmUserBoletoDebInc"
...
Public Sub ITEMBL_MontaMatriz()
  ...
  If lngBOLETOATENDID <> 0 Then
    ...
    For intI = 1 To 10  'varre as linhas
      ...
      strSql = "SELECT DEBITO.PKID "
      strSql = strSql & " FROM DEBITO " & _
              "WHERE DEBITO.NUMERO = " & Formata_Dados(intI, tpDados_Longo) & _
              " AND DEBITO.BOLETOATENDID = " & Formata_Dados(lngBOLETOATENDID, tpDados_Longo)
      '
...
Public Sub ITEMBLLANC_MontaMatriz()
  ...
  '
  'Cálculo Valor total
  strSql = "SELECT SUM(DEBITO.VALORPAGO) AS TOTAL "
  strSql = strSql & " FROM DEBITO " & _
          "WHERE DEBITO.BOLETOATENDID = " & Formata_Dados(lngBOLETOATENDID, tpDados_Longo)
  '
  ...
  '
  strSql = "SELECT DEBITO.PKID, DEBITO.BOLETOATENDID, DEBITO.NUMERO, EQUIPAMENTO.NUMERO, DEBITO.MEDICAO, DEBITO.VALORPAGO, (ISNULL(DEBITO.VALORPAGO,0) / ISNULL(DEBITO.COEFICIENTE,0)) AS CREDITO, DEBITO.DATA "
  strSql = strSql & " FROM DEBITO " & _
          ...
          "WHERE DEBITO.BOLETOATENDID = " & Formata_Dados(lngBOLETOATENDID, tpDados_Longo) & _
          " ORDER BY DEBITO.NUMERO;"
  '
  Set objRs = clsGer.ExecutarSQL(strSql)
  '
  If Not objRs.EOF Then
...
'''Private Sub cboBoletoDeb_Click()
'''  On Error GoTo trata
'''  Dim objBoletoDeb As busSisMaq.clsBoletoDeb
'''  Dim objRs As ADODB.Recordset
'''  'Alterna para status de alteração/inclusão
'''  If cboBoletoDeb.Text = "" Then
...
'''    Exit Sub
'''  End If
'''  Set objBoletoDeb = New busSisMaq.clsBoletoDeb
'''  Set objRs = objBoletoDeb.ListarBoletoDebPeloSeq(lngCONTRATOID, _
'''                                                  lngOBRAID, _
'''                                                  Left(cboBoletoDeb.Text, 3))
'''  If objRs.EOF Then
'''    TratarErroPrevisto "Devolução " & cboBoletoDeb.Text & " não cadastrada!"
'''    Status = tpStatus_Incluir
'''    lngBOLETOATENDID = 0
'''    Form_Load
'''  Else
'''    Status = tpStatus_Alterar
'''    lngBOLETOATENDID = objRs.Fields("PKID").Value
'''    Form_Load
'''  End If
'''  'Montar RecordSet
...
'''    Next
  Case 2 'Cadastrar Todos
    ...
    For intI = 0 To ITEMBL_LINHASMATRIZ - 1
      ...
      If grdBL.Columns("Máquina").Text & "" <> "" And _
          ...
          grdBL.Columns("Valor").Text & "" <> "" And _
          grdBL.Columns("Nro.").Text & "" <> "ok" Then
        'Propósito: Retornar todos os ítens
        '
        blnSelItem = True
        If Not ValidaCamposItemBLDestino(intI) Then
          blnEncontrouErro = True
          If lngRowSetErro = 0 Then lngRowSetErro = intI
        Else
          ...
          strSql = "SELECT ISNULL(SUM(ENTRADAATEND.VALOR), 0) AS VALOR " & _
                    ...
                    " WHERE CAIXAATEND.PKID = " & Formata_Dados(RetornaCodTurnoCorrenteAtend(lngFUNCIONARIOID, 0), tpDados_Longo)
          Set objRs = objGeral.ExecutarSQL(strSql)
          If Not objRs.EOF Then
            curSaldo = objRs.Fields("VALOR").Value
          End If
          objRs.Close
          Set objRs = Nothing
          '
          ...
          Set objRs = objGeral.ExecutarSQL(strSql)
          If Not objRs.EOF Then
            curSaldo = curSaldo - objRs.Fields("VALOR").Value
          End If
          objRs.Close
          Set objRs = Nothing
          '
          If CCur(grdBL.Columns("Valor").Text) > curSaldo Then
            TratarErroPrevisto "Valor lançado " & grdBL.Columns("Valor").Text & " não pode ser maior que o saldo " & Format(curSaldo, "###,##0.00") & " na linha " & grdBL.Columns("Nro.").Text & ". Esta linha não será lançada."
          Else
            ...
            VerificaStatusConsolicacao lngBOLETOATENDID
            'Indica se quantidade restante fechou
          End If
          grdBL.Columns("Nro.").Text = "ok"
          ITEMBL_Matriz(0, grdBL.Row) = "ok"
        End If
      End If
    Next
    ...
    If blnSelItem = False And blnEncontrouErro = False Then
      TratarErroPrevisto "Entre com ao menos 1 ítem do Boleto", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAllSel]"
      grdBL.Row = 0
      grdBL.ReBind
      grdBL.SetFocus
      Exit Sub
    End If
    '
    If blnEncontrouErro = False Then
      blnFechar = True
      Unload Me
    Else
      grdBL.Row = lngRowSetErro
      grdBL.ReBind
      grdBL.SetFocus
      Exit Sub
    End If
  Case 3 'Retirar Selecionados
    ...
    For intI = 0 To grdBLOrigem.SelBookmarks.Count - 1
      ...
      'excluir debito
      objDebito.ExcluirDebito grdBLOrigem.Columns("DEBITOID").Text
      'Verifica consolidação
      VerificaStatusConsolicacao lngBOLETOATENDID
    ...
    If blnSel = False Then
      TratarErroPrevisto "Nenhum ítem do boleto selecionado para exclusão.", "[frmUserBoletoDebInc.TratarAssociacao]"
    End If
...
'''      objBoletoDeb.AlterarEstoquePelaBoletoDeb grdBLOrigem.Columns("ESTOQUEID").Text, _
'''                                               grdBLOrigem.Columns("Devol.").Text, _
'''                                               "RET"
'''      objBoletoDeb.ExcluirItemDeVolucao grdBLOrigem.Columns("ITEMDEVOLUCAOID").Text
'''      'Verifica consolidação
...
'''    '
  If blnRet Then 'Houve Alteração, Atualiza grids
    blnRetorno = True
    '
    ITEMBL_COLUNASMATRIZ = grdBL.Columns.Count
    ITEMBL_LINHASMATRIZ = 0
...
'''    TratarErroPrevisto strMsg, "[frmUserBoletoDebInc.ValidaCamposItemNFGeral]"
...
'''             Err.Description, _
'''             "[frmUserBoletoDebInc.ValidaCamposItemNFGeral]"
'''  ValidaCamposItemNFGeral = False
'''End Function
'''
Private Function ValidaCamposItemBLDestino(intLinha As Long) As Boolean
  On Error GoTo trata
  Dim strMsg                As String
  Dim blnSetarFocoControle  As Boolean
  Dim strSql                As String
  ...
  End If
  'Validção da Medição
  If Not Valida_Moeda(grdBL.Columns("Medição"), TpObrigatorio, blnSetarFocoControle, blnPintarControle:=False, blnValidarPeloClip:=False) Then
  ...
  If Len(strMsg) = 0 Then
    ...
    If strMsg = "" Then
      ...
      strSql = "SELECT TOP 1 ISNULL(DEBITO.MEDICAO, 0) AS MED FROM DEBITO " & _
            " WHERE DEBITO.MAQUINAID = " & Formata_Dados(lngMAQUINAID, tpDados_Longo) & _
            " AND DEBITO.TURNOID = " & Formata_Dados(RetornaCodTurnoCorrente, tpDados_Longo) & _
            " ORDER BY DEBITO.PKID DESC "
      Set objRs = objGeral.ExecutarSQL(strSql)
      curMEDICAOANT = 0
      If Not objRs.EOF Then
      ...
      If curMEDICAOANT = 0 Then
        ...
        strSql = "SELECT ISNULL(LEITURAMAQUINAFECHA.MEDICAOSAIDA, 0) AS MED FROM LEITURAFECHA " & _
  ...
  '
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserBoletoDebInc.ValidaCamposItemBLDestino]"
...
trata:
  TratarErro Err.Number, _
             Err.Description, _
             "[frmUserBoletoDebInc.ValidaCamposItemBLDestino]"
  ValidaCamposItemBLDestino = False
End Function
...
Private Function ValidaCamposItemBLDestinoAllSel() As Boolean
  ...
  For intRows = 0 To ITEMBL_LINHASMATRIZ - 1
    ...
    '
    If grdBL.Columns(1).Text & "" <> "" Or grdBL.Columns(2).Text & "" <> "" Or grdBL.Columns(3).Text & "" <> "" Or grdBL.Columns(4).Text & "" <> "" Then
      'Selecionou um item
      blnSelItem = True
    End If
    If grdBL.Columns(1).Text & "" <> "" Or _
      grdBL.Columns(2).Text & "" <> "" Or _
      grdBL.Columns(3).Text & "" <> "" Or _
      grdBL.Columns(4).Text & "" <> "" Then
...
trata:
  TratarErro Err.Number, _
             Err.Description, _
             "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAllSel]"
...
'''    blnRetorno = False
'''    TratarErroPrevisto "Nenhum item lançado para esta NF.", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
'''  End If
'''  If blnRetorno = True Then
'''    'Nenhum erro encontrado
'''    If Not Valida_String(txtNFCliente, TpObrigatorio, False) Then
'''      TratarErroPrevisto "Número NF Cliente inválido.", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
'''      blnRetorno = False
'''    End If
'''  End If
'''  If blnRetorno = True Then
'''    If Not Valida_Data(mskData, TpObrigatorio, False) Then
'''      TratarErroPrevisto "Data inválida.", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
'''      blnRetorno = False
'''    End If
'''  End If
'''  If blnRetorno = True Then
'''    If Not Valida_Data(mskDataEmissao, TpObrigatorio, False) Then
'''      TratarErroPrevisto "Data de emissão inválida.", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
'''      blnRetorno = False
'''    End If
'''  End If
'''  If blnRetorno = True Then
'''    If Not Valida_Data(mskDataBoletoDeb, TpObrigatorio, False) Then
'''      TratarErroPrevisto "Data de devolução inválida.", "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
'''      blnRetorno = False
'''    End If
'''  End If
...
'''  TratarErro Err.Number, _
'''             Err.Description, _
'''             "[frmUserBoletoDebInc.ValidaCamposItemBLDestinoAll]"
...
trata:
  Err.Raise Err.Number, _
            "[frmUserBoletoDebInc.LimparCampos]", _
            Err.Description
End Sub
...
Private Sub Form_KeyPress(KeyAscii As Integer)
  ...
  If Me.ActiveControl.Name <> "grdBL" Then
    ...
    End If
  Else
    If grdBL.Columns(0).Text = "ok" Then
      KeyAscii = 0
    ElseIf KeyAscii = 13 And grdBL.Row <> -1 Then
      If grdBL.Col = 4 Then
        blnSairRow = True
        blnSairGrid = True
        '
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.Form_Activate]"
End Sub
...
'''    Set objBoletoDeb = New busSisMaq.clsBoletoDeb
'''    Set objRs = objBoletoDeb.ListarBoletoDeb(lngBOLETOATENDID)
'''    '
'''    If Not objRs.EOF Then
...
'''      INCLUIR_VALOR_NO_MASK mskDataBoletoDeb, objRs.Fields("DATADEVOLUCAO").Value & "", TpMaskData
...
'''  Exit Sub
'''trata:
'''  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.grdBL_BeforeRowColChange]"
'''End Sub
'''
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.grdGeral_UnboundReadDataEx]"
End Sub
...
''''''  Exit Sub
''''''trata:
''''''  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.grdBLOrigem_BeforeRowColChange]"
''''''End Sub
''''''
...
Private Sub grdBL_Validate(Cancel As Boolean)
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.grdBL_Validate]"
End Sub
...
Private Sub txtBoleto_LostFocus()
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserBoletoDebInc.txtSenha_LostFocus]"
...
Private Function ValidaCampos() As Boolean
  ...
  If Not Valida_Moeda(txtBoleto, TpObrigatorio, blnSetarFocoControle) Then
  ...
  If Len(strMsg) = 0 Then
    ...
    Set objGeral = New busSisMaq.clsGeral
    strSql = "Select BOLETOATEND.PKID "
    strSql = strSql & " FROM CAIXAATEND INNER JOIN BOLETOATEND ON CAIXAATEND.PKID = BOLETOATEND.CAIXAATENDID "
    strSql = strSql & " WHERE BOLETOATEND.NUMERO =  " & Formata_Dados(txtBoleto.Text, tpDados_Texto)
    strSql = strSql & " AND CAIXAATEND.ATENDENTEID =  " & Formata_Dados(lngFUNCIONARIOID, tpDados_Longo)
  
    Set objRs = objGeral.ExecutarSQL(strSql)
    'Verifica se o boleto existe para o usuário
    If objRs.EOF Then
      strMsg = strMsg & "Boleto não encontrado para este atendente"
  ...
  If Len(strMsg) <> 0 Then
    TratarErroPrevisto strMsg, "[frmUserBoletoDebInc.ValidaCampos]"
    ValidaCampos = False
  Else
    ValidaCampos = True
  End If
...
trata:
  Err.Raise Err.Number, _
            Err.Source & ".[frmUserBoletoDebInc.ValidaCampos]", _
            Err.Description
End Function
