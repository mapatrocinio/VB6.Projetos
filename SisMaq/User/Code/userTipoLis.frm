...
Begin VB.Form frmUserTipoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de Tipo"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8340
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userTipoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6330
   End
End
Attribute VB_Name = "frmUserTipoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione uma Tipo !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserTipoInc.lngTIPOID = grdGeral.Columns("ID").Value
  frmUserTipoInc.Status = tpStatus_Alterar
  frmUserTipoInc.Show vbModal
  
  If frmUserTipoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  ...
  If Len(Trim(grdGeral.Columns("ID").Value & "")) = 0 Then
    MsgBox "Selecione uma Tipo para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from MAQUINA where TIPOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Máquina não pode ser excluida, pois já está associada a maquinas.", "frmUserTipoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do Tipo " & grdGeral.Columns("Tipo").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  ...
  objTipo.ExcluirTipo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserTipoInc.Status = tpStatus_Incluir
  frmUserTipoInc.lngTIPOID = 0
  frmUserTipoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT TIPO.PKID, TIPO.TIPO " & _
            "FROM TIPO " & _
            "ORDER BY TIPO"
  '
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserTipoLis.grdGeral_UnboundReadDataEx]"
End Sub
