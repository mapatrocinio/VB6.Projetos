...
Begin VB.Form frmUserRetiradaLis 
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userRetiradaLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   4305
   End
End
Attribute VB_Name = "frmUserRetiradaLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma retirada !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserRetiradaInc.Status = tpStatus_Alterar
  frmUserRetiradaInc.lngPKID = grdGeral.Columns("PKID").Value
  frmUserRetiradaInc.Show vbModal
  
  If frmUserRetiradaInc.blnRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objRetirada          As busSisMaq.clsRetirada
  ...
  'Cadastro de Séries
  If Not IsNumeric(grdGeral.Columns("PKID").Value & "") Then
    MsgBox "Selecione uma retirada para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  'OK
  If MsgBox("Confirma exclusão da retirada de " & grdGeral.Columns("Data").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objRetirada = New busSisMaq.clsRetirada
  objRetirada.ExcluirRetirada CLng(grdGeral.Columns("PKID").Value)
...
Private Sub cmdInserir_Click()
  On Error GoTo trata
  frmUserRetiradaInc.Status = tpStatus_Incluir
  frmUserRetiradaInc.lngPKID = 0
  frmUserRetiradaInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  'Chamada do Cadastro de Retirada
  strSql = "SELECT RETIRADA.PKID, RETIRADA.DATA, RETIRADA.VALOR "
  strSql = strSql & " FROM RETIRADA " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserRetirada.grdGeral_UnboundReadDataEx]"
End Sub
