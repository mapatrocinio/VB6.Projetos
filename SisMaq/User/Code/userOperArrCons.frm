...
Begin VB.Form frmUserOperArrCons 
   ...
   Begin VB.Frame fraUnidade 
      ...
      Begin TrueDBGrid60.TDBGrid grdBoleto 
         Height          =   1740
         Left            =   120
         OleObjectBlob   =   "userOperArrCons.frx":95E0
         TabIndex        =   20
         TabStop         =   0   'False
         Top             =   1170
         Width           =   5820
...
Public Sub VerificaQuemChamou()
  ...
  Dim objCreditoInc As SisMaq.frmUserCreditoInc
  ...
  Case 0
    ...
    objUserBoletoCredInc.Status = Status
    objUserBoletoCredInc.lngTURNOARRECEPESQ = lngTURNOARRECEPESQ
  ...
  Case 1
    If Not IsNumeric(grdArrecadador.Columns("CREDITOID").Value) Then
      MsgBox "Selecione um lançamento !", vbExclamation, TITULOSISTEMA
      Exit Sub
    End If
    '
    Set objCreditoInc = New SisMaq.frmUserCreditoInc
    objCreditoInc.Status = tpStatus_Alterar
    objCreditoInc.lngPKID = grdArrecadador.Columns("CREDITOID").Value
    objCreditoInc.Show vbModal
    Set objCreditoInc = Nothing
  Case 2
    'Deslogar
    lngFUNCIONARIOID = 0
    txtUsuario.Text = ""
...
'''    'End If
'''    'Confirmação
'''    If MsgBox("Confirma cancelamento da GR " & grdArrecadador.Columns("Seq.").Value & " de " & grdArrecadador.Columns("Prontuário").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
'''      SetarFoco grdArrecadador
'''      Exit Sub
'''    End If
'''
...
'''    End If
'''    'Confirmação
'''    If MsgBox("Confirma impressão da GR " & grdArrecadador.Columns("Seq.").Value & " de " & grdArrecadador.Columns("Prontuário").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
'''      SetarFoco grdArrecadador
'''      Exit Sub
'''    End If
...
'''  End If
  ...
  ElseIf Status = tpStatus_Consultar Then
    ...
    'Obter usuário e senha do turno corrente
    Set objGeral = New busSisMaq.clsGeral
    strSql = "SELECT FUNCIONARIO.USUARIO, FUNCIONARIO.SENHA FROM FUNCIONARIO " & _
          " INNER JOIN CAIXAARREC ON FUNCIONARIO.PESSOAID = CAIXAARREC.ARRECADADORID " & _
          " WHERE CAIXAARREC.PKID = " & Formata_Dados(lngTURNOARRECEPESQ, tpDados_Longo)
...
Public Sub MontaRESBOL_Matriz()
  ...
  '
  strSql = "SELECT BOLETOARREC.PKID, MAX(PESSOA.NOME), BOLETOARREC.NUMERO, SUM(CREDITO.VALORPAGO) AS TOTAL, MAX(BOLETOARREC.STATUS), ISNULL(COUNT(CREDITO.PKID),0) AS LANCADO, 10 - ISNULL(COUNT(CREDITO.PKID),0) AS ALANC "
...
Public Sub MontaARREC_Matriz()
  ...
  Set clsGer = New busSisMaq.clsGeral
  '
  strSql = "SELECT CREDITO.MAQUINAID, CREDITO.BOLETOARRECID, CREDITO.PKID, PESSOA.NOME, BOLETOARREC.NUMERO, CREDITO.NUMERO, EQUIPAMENTO.NUMERO, CREDITO.MEDICAO, CREDITO.VALORPAGO, CREDITO.COEFICIENTE, (ISNULL(CREDITO.VALORPAGO,0) / ISNULL(CREDITO.COEFICIENTE,0)) AS CREDITO, CREDITO.DATA "
  strSql = strSql & " FROM CREDITO " & _
          " INNER JOIN BOLETOARREC ON BOLETOARREC.PKID = CREDITO.BOLETOARRECID " & _
          " INNER JOIN MAQUINA ON MAQUINA.PKID = CREDITO.MAQUINAID " & _
          " INNER JOIN EQUIPAMENTO ON EQUIPAMENTO.PKID = MAQUINA.EQUIPAMENTOID " & _
