...
Begin VB.Form frmUserPeriodoLis 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lista de períodos"
   ClientHeight    =   4980
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8385
   ...
   Begin TrueDBGrid60.TDBGrid grdGeral 
      ...
      Height          =   4980
      Left            =   0
      OleObjectBlob   =   "userPeriodoLis.frx":0000
      TabIndex        =   0
      Top             =   0
      Width           =   6390
   End
End
Attribute VB_Name = "frmUserPeriodoLis"
...
Private Sub cmdAlterar_Click()
  ...
  If Not IsNumeric(grdGeral.Columns("ID").Value) Then
    MsgBox "Selecione um período !", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  
  frmUserPeriodoInc.lngPERIODOID = grdGeral.Columns("ID").Value
  frmUserPeriodoInc.Status = tpStatus_Alterar
  frmUserPeriodoInc.Show vbModal
  
  If frmUserPeriodoInc.bRetorno Then
    MontaMatriz
...
Private Sub cmdExcluir_Click()
  Dim objPeriodo As busSisMaq.clsPeriodo
  ...
  If Len(Trim(grdGeral.Columns("ID").Value)) = 0 Then
    MsgBox "Selecione um Período para exclusão.", vbExclamation, TITULOSISTEMA
    SetarFoco grdGeral
    Exit Sub
  End If
  '
  ...
  '
  strSql = "Select * from TURNO where PERIODOID = " & grdGeral.Columns("ID").Value
  Set objRs = objGer.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    objRs.Close
    Set objRs = Nothing
    Set objGer = Nothing
    TratarErroPrevisto "Período não pode ser excluido, pois já está associado a turnos.", "frmUserPeriodoLis.cmdExcluir_Click"
    SetarFoco grdGeral
    Exit Sub
  End If
  objRs.Close
  ...
  '
  Set objGer = Nothing
  If MsgBox("Confirma exclusão do Período " & grdGeral.Columns("Período").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
    SetarFoco grdGeral
    Exit Sub
  End If
  'OK
  Set objPeriodo = New busSisMaq.clsPeriodo
  
  objPeriodo.ExcluirPeriodo CLng(grdGeral.Columns("ID").Value)
  '
  MontaMatriz
  grdGeral.Bookmark = Null
  grdGeral.ReBind
...
Private Sub cmdInserir_Click()
  ...
  frmUserPeriodoInc.Status = tpStatus_Incluir
  frmUserPeriodoInc.lngPERIODOID = 0
  frmUserPeriodoInc.Show vbModal
...
Public Sub MontaMatriz()
  ...
  '
  strSql = "SELECT PERIODO.PKID, PERIODO.PERIODO, PERIODO.INICIO, PERIODO.TERMINO " & _
            "FROM PERIODO " & _
...
' nonnegative values of IndexFromBookmark() are valid.
  ...
  Exit Sub
trata:
  TratarErro Err.Number, Err.Description, "[frmUserPeriodoLis.grdGeral_UnboundReadDataEx]"
End Sub
