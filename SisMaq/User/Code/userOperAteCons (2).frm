...
Begin VB.Form frmUserOperAteCons 
   ...
   Begin VB.Frame fraUnidade 
      ...
      Begin TrueDBGrid60.TDBGrid grdBoleto 
         Height          =   1740
         Left            =   5910
         OleObjectBlob   =   "userOperAteCons.frx":DBC7
         TabIndex        =   25
         TabStop         =   0   'False
         Top             =   1170
         Width           =   5820
...
'''    'End If
'''    'Confirmação
'''    If MsgBox("Confirma cancelamento da GR " & grdAtendente.Columns("Seq.").Value & " de " & grdAtendente.Columns("Prontuário").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
'''      SetarFoco grdAtendente
'''      Exit Sub
'''    End If
'''
...
'''    End If
'''    'Confirmação
'''    If MsgBox("Confirma impressão da GR " & grdAtendente.Columns("Seq.").Value & " de " & grdAtendente.Columns("Prontuário").Value & "?", vbYesNo, TITULOSISTEMA) = vbNo Then
'''      SetarFoco grdAtendente
'''      Exit Sub
'''    End If
...
Public Sub Trata_Matrizes_Totais()
  ...
  strSql = "SELECT ISNULL(SUM(ENTRADAATEND.VALOR), 0) AS VALOR " & _
            ...
            " WHERE CAIXAATEND.PKID = " & Formata_Dados(RetornaCodTurnoCorrenteAtend(lngFUNCIONARIOID), tpDados_Longo)
...
Public Sub MontaRESBOL_Matriz()
  ...
  strSql = strSql & " FROM " & _
          ...
          " WHERE CAIXAATEND.PKID = " & Formata_Dados(RetornaCodTurnoCorrenteAtend(lngFUNCIONARIOID), tpDados_Longo) & _
...
Private Function ValidaCampos() As Boolean
  ...
  End If
  If Len(strMsg) = 0 Then
    lngTURNOATENDID = RetornaCodTurnoCorrenteAtend(lngFUNCIONARIOID)
