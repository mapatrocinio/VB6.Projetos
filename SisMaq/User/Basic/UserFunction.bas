...
Private Function ValidaCamposFechaTurno() As Boolean
  ...
  '
  'CAIXA ATEND ABERTO
  strSql = "SELECT FUNCIONARIO.USUARIO FROM CAIXAATEND " & _
    " INNER JOIN FUNCIONARIO ON FUNCIONARIO.PESSOAID = CAIXAATEND.ATENDENTEID " & _
    " WHERE CAIXAATEND.TURNOFECHAID IS NULL "
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    strMsg = strMsg & "Há Turnos abertos para atendente. Feche o turno dos atendentes e após feche o turno." & vbCrLf
    blnSetarFocoControle = False
    Do While Not objRs.EOF
      strMsg = strMsg & objRs.Fields("USUARIO").Value & vbCrLf
      objRs.MoveNext
    Loop
  ...
  'CAIXA ARREC ABERTO
  strSql = "SELECT FUNCIONARIO.USUARIO FROM CAIXAARREC " & _
    " INNER JOIN FUNCIONARIO ON FUNCIONARIO.PESSOAID = CAIXAARREC.ARRECADADORID " & _
    " WHERE CAIXAARREC.TURNOFECHAID IS NULL"
  Set objRs = objGeral.ExecutarSQL(strSql)
  If Not objRs.EOF Then
    strMsg = strMsg & "Há Turnos abertos para arrecadadores. Feche o turno dos arrecadadores e após feche o turno." & vbCrLf
    blnSetarFocoControle = False
    Do While Not objRs.EOF
      strMsg = strMsg & objRs.Fields("USUARIO").Value & vbCrLf
      objRs.MoveNext
  ...
  If Len(strMsg) = 0 Then
    'Verifica se quem está fechando o turno é realmente quem abriu o turno
    strSql = "SELECT TURNO.*, PERIODO.PERIODO " & _
      "FROM TURNO " & _
      " INNER JOIN PERIODO ON PERIODO.PKID = TURNO.PERIODOID " & _
    ...
    If Not objRs.EOF Then
      ...
      lngPeriodo = objRs.Fields("PERIODO").Value
      If giFuncionarioId <> objRs.Fields("CAIXAID").Value Then
        'Funcionário logado não é o mesmo que está fechando o turno
        ...
        If Len(Trim(gsNomeUsuLib)) = 0 Then
          strMsg = "É necessário confirmação do funcionário que abriu o turno para executar esta ação."
        End If
      End If
    End If
    objRs.Close
    Set objRs = Nothing
  End If
  'NOVO obrigar a lançar leitura especial final para o turno
  If Len(strMsg) = 0 Then
    'Verifica se FOI LANÇADO LEITURA ESPECIAL FINAL
    lngCorTurnoCorrente = RetornaCodTurnoCorrente(datTurnoCorrente)
    ...
    strSql = "SELECT * " & _
      ...
      "WHERE LEITURAFECHA.DATA >= " & Formata_Dados(strDATAINI, tpDados_DataHora) & _
      " AND LEITURAFECHA.DATA < " & Formata_Dados(strDATAFIM, tpDados_DataHora) & _
      " AND PERIODO.PERIODO = " & Formata_Dados(lngPeriodo, tpDados_Longo) & _
      " AND LEITURAFECHA.STATUS = " & Formata_Dados("F", tpDados_Texto)
    Set objRs = objGeral.ExecutarSQL(strSql)
    If objRs.EOF Then
      strMsg = "É necessário entrar com a leitura final antes de fechar o turno."
    End If
    objRs.Close
    Set objRs = Nothing
  End If
...
'Proposito: Verificar consolidação da BOLETOATEND
...
'Proposito: Verificar consolidação da BOLETOARREC
...
Public Function RetornaCodTurnoCorrente(Optional datData As Date, _
                                        Optional lngPeriodo As Long) As Long
  On Error GoTo trata
  'Retorna 0 - para Código de Erro
  Dim strSql        As String
  Dim objRs         As ADODB.Recordset
  ...
  Else
    ...
    lngPeriodo = objRs.Fields("PERIODOID").Value
  End If
  objRs.Close
  Set objRs = Nothing
  Set objGeral = Nothing
...
'Proposito: Retornar o Código do Turno Corrente
Public Function RetornaCodTurnoCorrenteArrec(lngARRECADADORID As Long, _
                                             lngTURNOARRECID As Long) As Long
  On Error GoTo trata
  'Retorna 0 - para Código de Erro
  Dim strSql        As String
  Dim objRs         As ADODB.Recordset
  ...
  If lngTURNOARRECID <> 0 Then
    RetornaCodTurnoCorrenteArrec = lngTURNOARRECID
  Else
...
'Proposito: Retornar o Código do Turno Corrente
Public Function RetornaCodTurnoCorrenteAtend(lngATENDENTEID As Long, _
                                             lngTURNOATENDID As Long) As Long
  On Error GoTo trata
  'Retorna 0 - para Código de Erro
  Dim strSql        As String
  Dim objRs         As ADODB.Recordset
  ...
  If lngTURNOATENDID <> 0 Then
    RetornaCodTurnoCorrenteAtend = lngTURNOATENDID
  Else
